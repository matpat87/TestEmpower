...
class ListViewData
{
    ...
    public function getListViewData($seed, $where, $offset=-1, $limit = -1, $filter_fields=array(), $params=array(), $id_field = 'id', $singleSelect=true, $id = null)
    {
        ...
        if (!empty($params['overrideOrder']) && !empty($params['orderBy'])) {
            // APX Custom Codes: Fixed global issue when switching between the two fields for sorting the second click for descending still displays ascending. In essence showing the same list twice. (If empty($params['sortOrder'] return as 'ASC')
            $order = $this->getOrderBy(strtolower($params['orderBy']), (empty($params['sortOrder']) ? 'ASC' : $params['sortOrder'])); // retreive from $_REQUEST
        } else {
