...
function get_sugar_config_defaults(): array
{
    ...
    $sugar_config_defaults = [
        ...
        'email_default_client' => 'mailto', // APX Custom Codes: Changed from sugar to mailto
...
function get_user_array($add_blank = true, $status = 'Active', $user_id = '', $use_real_name = false, $user_name_filter = '', $portal_filter = ' AND portal_only=0 ', $from_cache = true)
{
    ...
    if (empty($user_array)) {
        ...
        // APX Custom code: included `status` in selected columns
        if (empty($status)) {
            $query = 'SELECT users.id, users.first_name, users.last_name, users.user_name, users.status FROM users WHERE 1=1' . $portal_filter;
        } else {
            $query = "SELECT users.id, users.first_name, users.last_name, users.user_name, users.status from users WHERE users.status='$status'" . $portal_filter;
        }
        ...
        if (!is_admin($current_user)) {
            ...
            if ((empty($_REQUEST['module']) || $_REQUEST['module'] == 'Home') && (empty($_REQUEST['action']) || $_REQUEST['action'] == 'DynamicAction')) {
                $group_where = SecurityGroup::getGroupUsersWhere($current_user->id);
            } else {
            ...
            // APX Custom Code:  if user is an admin, include query for users with Inactive status
            if ($_REQUEST['searchFormTab'] = 'advanced_search') {
                $query = str_replace("WHERE users.status='Active'", "WHERE users.status IN ('Active', 'Inactive')", $query);
            }
        }
        ...
        if (!empty($user_name_filter)) {
            ...
            $query .= " AND users.user_name LIKE '$user_name_filter%' ";
        }
        if (!empty($user_id)) {
            $query .= " OR users.id='{$user_id}'";
        }

        //get the user preference for name formatting, to be used in order by
        $order_by_string = ' users.status, users.user_name ASC ';
        if (!empty($current_user) && !empty($current_user->id)) {
            ...
            if ($firstNamePos !== false || $lastNamePos !== false) {
                ...
                if ($firstNamePos === false) {
                    $order_by_string = 'users.status, users.last_name ASC';
                } else {
                    $order_by_string = ($lastNamePos < $firstNamePos) ? 'users.status, users.last_name, users.first_name ASC' : 'users.status, users.first_name, users.last_name ASC';
                }
            }
        }
        ...
        // Custom codes to modify auto-complete of User relate fields in Accounts Module (SAM and MDM) to filter based on ACL Roles Module
        // User.php create_new_list_query does not work for auto-complete which is why changes are made on the core utils.php file
        if ($_REQUEST['action'] == 'quicksearchQuery') {
            if (strpos($_REQUEST['data'], 'users_accounts_1_name') !== false) {
                $query = str_replace(
                    "WHERE",
                ...
                INNER JOIN acl_roles 
                    ON acl_roles_users.role_id = acl_roles.id
                        ...
                        AND acl_roles.name = 'Strategic Account Manager' WHERE",
                    $query
                );
            }

            if (strpos($_REQUEST['data'], 'users_accounts_2_name') !== false) {
                $query = str_replace(
                    "WHERE",
                    "INNER JOIN acl_roles_users 
                    ON acl_roles_users.user_id = users.id 
                ...
                INNER JOIN acl_roles 
                    ON acl_roles_users.role_id = acl_roles.id
                        AND acl_roles.deleted = 0
                        AND acl_roles.name = 'Market Development Manager' WHERE",
                    $query
                );
            }

            /* Custom Codes - For Customer Service Relate field (accounts): Ontrack 1795 */
            $requestDataDecoded = json_decode(html_entity_decode($_REQUEST['data']), true);
            if (in_array('users_accounts_3_name', $requestDataDecoded['populate_list'])) {

                $query = str_replace(
                    "WHERE",
                    " WHERE users.department='CustomerService' AND users.status='Active' AND ",
                    $query);
            }
        }
        ...
        while ($row = $db->fetchByAssoc($result)) {
            //       APX Custom Code: to display custom Inactive text for inactive users
            $statusStr = ($row['status'] == 'Inactive') ? ' (INACTIVE)' : '';
            if ($use_real_name == true || showFullName()) {
                if (isset($row['last_name'])) { // cn: we will ALWAYS have both first_name and last_name (empty value if blank in db)
                    $temp_result[$row['id']] = $locale->getLocaleFormattedName($row['first_name'], $row['last_name']) . "{$statusStr}";
                } else {
                    $temp_result[$row['id']] = $row['user_name'] . "{$statusStr}";
                }
            } else {
                $temp_result[$row['id']] = $row['user_name'] . "{$statusStr}";
            }
...
function getUserArrayFromFullName($args, $hide_portal_users = false)
{
    ...
    if (strpos($args, ' ')) {
        ...
        foreach ($argArray as $arg) {
            $arg = $db->quote($arg);
            $inClauses[] = "(users.first_name LIKE '{$arg}%' OR users.last_name LIKE '{$arg}%')";
        }

        $inClause = '(' . implode('OR ', $inClauses) . ')';
    } else {
        $args = $db->quote($args);
        $inClause = "(users.first_name LIKE '{$args}%' OR users.last_name LIKE '{$args}%')";
    }
    // ~jmorais@dri

    $query = "SELECT users.id, users.first_name, users.last_name, users.user_name, users.status FROM users WHERE users.status='Active' AND users.deleted=0 AND ";
    if ($hide_portal_users) {
        $query .= ' users.portal_only=0 AND ';
    }
    $query .= $inClause;
    /* BEGIN - SECURITY GROUPS */
    global $current_user, $sugar_config;
...
function get_select_options_with_id_separate_key($label_list, $key_list, $selected_key, $massupdate = false)
{
    ...
    foreach ($key_list as $option_key => $option_value) {
        ...
        // APX Custom Codes: Added $option_key != 0 condition and changed all === AND !== to == AND != conditions to fix issue where it forces 0 values to be selected for OnTrack Ticket 660
        if (
                ($option_key != '' && $selected_key == $option_key) || (
                $option_key != 0 && $option_key == '' && (($selected_key == '' && !$massupdate) || $selected_key == '__SugarMassUpdateClearField__')
