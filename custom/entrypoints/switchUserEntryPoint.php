 ...
 */
 ...
 if (empty($_SESSION['sudo_user_ids'])) {
  $_SESSION['sudo_user_ids'] = [];  
}
...
if(!empty($_GET['user_id']) && $GLOBALS['current_user']->is_admin)
{
  ...
  if(!empty($user_focus->id))
  {
    if(empty($_SESSION['sudo_user_id'])) {
      ...
      $_SESSION['sudo_user_ids'][] = $GLOBALS['current_user']->id;
    }
...
elseif(!empty($_GET['back_to_sudo']) && !empty($_SESSION['sudo_user_id']) && isset($_GET['back_to_admin']))
{
  if (!empty($_SESSION['sudo_user_ids']) ) {
    $_SESSION['sudo_user_id'] = $_SESSION['sudo_user_ids'][0];
    unset($_SESSION['sudo_user_ids']);
  }
  ...
  // {
    ...
    $GLOBALS['log']->fatal(print_r($_SESSION['sudo_user_ids'], true));
    $GLOBALS['current_user'] = $user_focus;
    $_SESSION['authenticated_user_id'] = $user_focus->id;
