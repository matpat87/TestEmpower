...
class CalendarDisplay
{
    ...
    {
        global $sugar_config, $current_user; // APX Custom Codes
        ...
        $this->views = $views;
        $this->calendar_user = BeanFactory::getBean('Users', $current_user->getPreference('calendar_user_filter_id')); // APX Custom Codes
    }
    ...
    public function display()
    {
        ...
        $ss->assign('CALENDAR_FDOW', $this->calendar_user->get_first_day_of_week());
        ...
        $ss->assign('start_weekday', $this->calendar_user->get_first_day_of_week());
        // APX Custom Codes -- END
        ...
        $start = $this->calendar_user->getPreference('day_start_time');
        // APX Custom Codes -- END
        if (is_null($start)) {
        ...
        $end = $this->calendar_user->getPreference('day_end_time');
        // APX Custom Codes -- END
        if (is_null($end)) {
        ...
        $date_format = $this->convertPHPToMomentFormat($this->calendar_user->getPreference('datef') . " " . $this->calendar_user->getPreference('timef'));
        // APX Custom Codes -- END
        ...
        if ($this->cal->enable_repeat) {
            ...
            $fdow = $this->calendar_user->get_first_day_of_week();
            // APX Custom Codes -- END
            $dow = array();
    ...
    public function checkActivity($activity = "")
    {
        ...
        $newActivities = unserialize(base64_decode($this->calendar_user->getPreference("CalendarActivities")));
        // APX Custom Codes -- END
        if ($newActivities) {
    ...
    protected function load_settings_template(&$ss)
    {
        ...
        $USER_FILTER = retrieveSalesGroupUsers(); // APX Custom Codes
        
        // APX Custom Codes -- START
        // $displayTimeslots = $GLOBALS['current_user']->getPreference('calendar_display_timeslots');
        $displayTimeslots = $this->calendar_user->getPreference('calendar_display_timeslots');
        // APX Custom Codes -- END
        ...
        $shared_calendar_separate = $this->calendar_user->getPreference('calendar_display_shared_separate');
        // APX Custom Codes -- END
        ...
        // APX Custom Codes -- START
        $ss->assign('USER_FILTER', $USER_FILTER);
        $ss->assign('CALENDAR_USER_FILTER_ID', $this->calendar_user->id);
        // APX Custom Codes -- END
    }
    ...
    public function get_date_info($view, $date_time)
    {
        ...
        $dateFormat = $this->calendar_user->getUserDateTimePreferences();
        // APX Custom Codes -- END
    ...
    protected function get_next_calendar()
    {
        ...
        // APX Custom Codes -- START
        if ($this->cal->dashlet) {
			$str= str_replace('<svg', '<svg style="background-color: var(--custom-panel-bg); margin-top: 6px; margin-right: 5px"', $str);
        }
        // APX Custom Codes -- END
    ...
    protected function get_previous_calendar()
    {
        ...
        // APX Custom Codes -- START
        if ($this->cal->dashlet) {
			$str= str_replace('<svg', '<svg style="background-color: var(--custom-panel-bg); margin-top: 6px; margin-left: 5px"', $str);
        }
        // APX Custom Codes -- END
    ...
    public function display_shared_html($view)
    {
        ...
        $ss->assign("users_options", get_select_options_with_id(retrieveSalesGroupUsers($this->calendar_user->id), $this->cal->shared_ids));
        // APX Custom Codes -- END
