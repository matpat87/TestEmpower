...
{
    global $log; // APX Custom Codes: OnTrack #644 - Force Change Pass
    ...
    if (($current_user->portal_only=='0') && ($current_user->system_generated_password=='1')) {
        ...
        if ($type != '') {
            ...
            case '0':
                return false;
                break;
            // APX Custom Codes: OnTrack #644 - Force Change Pass -- START
            case '3':
                $log->info("is_reset_password_c: {$current_user->is_reset_password_c}");
                if($current_user->is_reset_password_c){
                    $log->info("{$current_user->user_name} needs to change password");
                    return true;
                }
            ...
            // APX Custom Codes: OnTrack #644 - Force Change Pass -- END
        }
    }
}
