...
function export($type, $records = null, $members = false, $sample=false)
{
    ...
    // APX Custom Codes: OnTrack #1309 - Fix for export and record count of listview -- START
    $i = 0;
    $custom_export_modules = array('ODR_SalesOrders', 'AOS_Invoices', 'CI_CustomerItems', 'Contacts', 'Calls', 'Meetings');
    // APX Custom Codes: OnTrack #1309 - Fix for export and record count of listview -- END
    ...
    if ($populate) {
        ...
        while ($val = $db->fetchByAssoc($result, false)) {
            ...
            // APX Custom Codes: OnTrack #1309 - Fix for export and record count of listview -- START
            if(in_array($focus->module_name, $custom_export_modules)){
                $pre_id = $val['id'] . $i;
            }
...
function get_field_order_mapping($name='', $reorderArr = '', $exclude = true)
{
    ...
    $field_order_array['opportunities'] = array( 'name' => 'Opportunity Name', 'id'=>'ID', 'amount' => 'Opportunity Amount', 'currency_id' => 'Currency', 'date_closed' => 'Expected Close Date', 'sales_stage' => 'Sales Stage', 'next_step' => 'Next Step', 'opportunity_type' => 'Opportunity Type', 'account_name' => 'Account Name', 'description' => 'Description', 'amount_usdollar' => 'Amount', 'lead_source' => 'Lead Source', 'campaign_id' => 'campaign_id', 'assigned_user_name' => 'Assigned User Name', 'assigned_user_id' => 'Assigned User ID', 'team_name' => 'Teams', 'team_id' => 'Team id', 'team_set_id' => 'Team Set ID', 'date_entered' => 'Date Created', 'date_modified' => 'Date Modified', 'created_by' => 'Created By ID', 'modified_user_id' => 'Modified By ID', 'deleted' => 'Deleted');
    ...
    // APX Custom Codes -- START
    $fields_to_exclude['opportunities'] = array('probability', 'amount_usdollar');
    // APX Custom Codes -- END
