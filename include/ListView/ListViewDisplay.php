...
class ListViewDisplay
{
    ...
    protected function buildActionsLink($id = 'actions_link', $location = 'top')
    {
        ...
        if (isset($this->templateMeta['form']['actions'])) {
            ...
            // APX Custom Codes: Removed as button will only be generated if $menuItems array values are not empty
            // Bulk Action label
            ...
            if (
                ...
                // APX Custom Codes: Do not include in Array if empty so that we can filter and display the action button only if $menuItems is not empty
                // $menuItems[] = "<a style='display:none'></a>"; 
            }
        }

        // APX Custom Codes: Check if $menuItems array values are not empty before generating the Bulk Action Button
        if (array_filter($menuItems)) {
            $menuItems = [$this->buildBulkActionButton($location)] + $menuItems;
        }
