...
class SugarFieldHandler
{
    ...
    public static function displaySmarty($parentFieldArray, $vardef, $displayType, $displayParams = array(), $tabindex = 1)
    {
        ...
        // APX Custom Codes: Fix on saving fields that have HTML characters: If the field type is html, change it to varchar -- START
        if (isset($vardef['type']) && $vardef['type'] == 'html') {
            $vardef['type'] = 'varchar';
        }
        // APX Custom Codes: Fix on saving fields that have HTML characters: If the field type is html, change it to varchar -- END
