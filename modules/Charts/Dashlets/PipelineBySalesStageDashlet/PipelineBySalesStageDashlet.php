...
class PipelineBySalesStageDashlet extends DashletGenericChart
{
    ...
    public $pbss_users = array(); // APX Custom Codes
    private $currency;
    ...
    public function displayOptions()
    {
        ...
        $this->_searchFields['pbss_users']['options'] = getUserRepresentativesForReports(); // APX Custom Codes
    ...
    public function display()
    {
        ...
        $assignedUserIds = json_encode($this->pbss_users); // APX Custom Codes
        ...
        $assignedUserIdsInputString = $this->parseAssignedUserIdsParam(); // APX Custom Code(s): Method to handle generating html string for a hidden input field for each assigned user id selected
...
resizeGraph(document.getElementById('$canvasId')); // APX Custom Codes: Fixed core issue where on filter, chart width exceeds the dashlet container's width
        </script>
EOD;
    ...
    */
    protected function constructQuery()
    {
        ...
        $query = "SELECT sales_stage, count(*) AS opp_count, sum(amount/1000) AS total
            FROM (
                ...
                WHERE opportunities.deleted = 0
                    AND opportunities.date_closed >= ".DBManagerFactory::getInstance()->convert("'".$this->pbss_date_start."'",'date'). " 
                    AND opportunities.date_closed <= ".DBManagerFactory::getInstance()->convert("'".$this->pbss_date_end."'",'date');
        
        if ( isset($this->pbss_sales_stages) && count($this->pbss_sales_stages) > 0 ) {
            $query .= " AND opportunities.sales_stage IN ('" . implode("','",$this->pbss_sales_stages) . "') ";
        }

        if ( isset($this->pbss_users) && count($this->pbss_users) > 0 ) {
//            $query .= " AND securitygroups_users.user_id IN ('" . implode("','",$this->pbss_users) . "') ";
            $query .= " AND opportunities.assigned_user_id IN ('" . implode("','",$this->pbss_users) . "') ";
        } else {
            $query .=  (! $current_user->is_admin) ? "AND securitygroups_users.user_id = '{$current_user->id}'" : '';
        }
    ...
    /* APX Custom Code: Method to handle generating html string for a hidden input field for each assigned user id selected -- START */
    protected function parseAssignedUserIdsParam(): string
    {
       $inputStr = "";
       
       // loop $this->pbss_users and generate an html input hidden string
        foreach ($this->pbss_users as $assignedUserId) {
            $inputStr .= "<input type='hidden' class='assignedUserIds' value='$assignedUserId' />";
        }
         return $inputStr;
    }
    /* APX Custom Code: Method to handle generating html string for a hidden input field for each assigned user id selected -- END */
}
