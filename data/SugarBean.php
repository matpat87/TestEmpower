...
class SugarBean
{
    ...
    {
        // APX Custom Codes: Fixed issue where the CRM log receives an error of "SugarBean::populateDefaultValues $field_defs should be an array" everytime a user goes to any edit view page -- START
        if (!$this->field_defs) {
            return;
        }
        // APX Custom Codes: Fixed issue where the CRM log receives an error of "SugarBean::populateDefaultValues $field_defs should be an array" everytime a user goes to any edit view page -- END
    ...
    protected function parseDateDefault($value, $time = false)
    {
        ...
        /* APX Custom Codes: added checker for $timedate if its value before triggering getNow(true)
            as it will result to a blank page if $timedate is not set
        */
        $now = isset($timedate) ? $timedate->getNow(true) : null;
    ...
    public function save($check_notify = false)
    {
        ...
        // APX Custom Codes: Force new assignment email notification to not trigger -- START
        $check_notify = isset($_REQUEST['check_notify']) ? $_REQUEST['check_notify'] : $check_notify;
        // APX Custom Codes: Force new assignment email notification to not trigger -- END
    ...
    public function create_notification_email($notify_user)
    {
        ...
        // APX Custom Codes: OnTrack #166 - This custom code allows us to globally append a "This is a test" string when testing in QA -- START
        $xtpl->assign("FOR_QA_EMAIL", $sugar_config['isQA'] == true ? '***This is a test from the Empower QA System***' : '');
        // APX Custom Codes: OnTrack #166 - This custom code allows us to globally append a "This is a test" string when testing in QA -- END
    ...
    public function create_new_list_query(
        ...
        // APX Custom Codes: Remove street address on account relate fields when triggering search via list view -- START
        if (   (isset($_REQUEST['account_name_advanced']) && $_REQUEST['account_name_advanced'])
            ||  (isset($_REQUEST['parent_name_advanced']) && $_REQUEST['parent_type_advanced'] == 'Accounts')
            ||  (isset($_REQUEST['account_c_advanced']) && $_REQUEST['account_c_advanced'])
            ||  (isset($_REQUEST['ci_customeritems_accounts_name_advanced']) && $_REQUEST['ci_customeritems_accounts_name_advanced'])
            ||  (isset($_REQUEST['accounts_rrq_regulatoryrequests_1_name_advanced']) && $_REQUEST['accounts_rrq_regulatoryrequests_1_name_advanced'])
            ||  (isset($_REQUEST['tr_technicalrequests_accounts_name_advanced']) && $_REQUEST['tr_technicalrequests_accounts_name_advanced'])
            ||  (isset($_REQUEST['billing_account_advanced']) && $_REQUEST['billing_account_advanced'])
            ||  (isset($_REQUEST['accounts_odr_salesorders_1_name_advanced']) && $_REQUEST['accounts_odr_salesorders_1_name_advanced'])
            ||  (isset($_REQUEST['apr_account_name_nondb_advanced']) && $_REQUEST['apr_account_name_nondb_advanced'])
        ) {
            $accountNameRequest = $_REQUEST['account_name_advanced'] ?? $_REQUEST['parent_name_advanced'];
            $accountNameRequest = $accountNameRequest ?? $_REQUEST['account_c_advanced'];
            $accountNameRequest = $accountNameRequest ?? $_REQUEST['ci_customeritems_accounts_name_advanced'];
            $accountNameRequest = $accountNameRequest ?? $_REQUEST['accounts_rrq_regulatoryrequests_1_name_advanced'];
            $accountNameRequest = $accountNameRequest ?? $_REQUEST['tr_technicalrequests_accounts_name_advanced']; //Ontrack #1001 - TR - Account
            $accountNameRequest = $accountNameRequest ?? $_REQUEST['billing_account_advanced']; //OnTrack #1171
            $accountNameRequest = $accountNameRequest ?? $_REQUEST['accounts_odr_salesorders_1_name_advanced']; //OnTrack #1170
            $accountNameRequest = $accountNameRequest ?? $_REQUEST['account_c_advanced']; //OnTrack #1170
            $accountNameRequest = $accountNameRequest ?? $_REQUEST['apr_account_name_nondb_advanced'];
            $formattedAccountName = explode(" (", $accountNameRequest)[0] ?? $accountNameRequest;
            $ret_array['where'] = str_replace($accountNameRequest, $formattedAccountName, $ret_array['where']);
        }
        // APX Custom Codes: Remove street address on account relate fields when triggering search via list view -- END
    ...
    public function create_list_count_query($query)
    {
        ...
        if (substr_count(strtolower($query), 'distinct')) {
            if (!empty($this->seed) && !empty($this->seed->table_name)) {
                // APX Custom Codes: Fix Campaign Log Subpanel Count -- START
                if ($this->seed->table_name == 'campaign_log') {
                    $star = 'DISTINCT ' . $this->seed->table_name . '.target_id';
                } else {
                    $star = 'DISTINCT ' . $this->seed->table_name . '.id';
                }
                // APX Custom Codes: Fix Campaign Log Subpanel Count -- END
            } else {
    ...
    public function get_list_view_array()
    {
        ...
        // APX Custom Codes: Append street address on account relate fields on both edit and detail views -- START
        if ($this->account_id) {
            $account = BeanFactory::getBean('Accounts', $this->account_id);
            $return_array['ACCOUNT_NAME'] = ($account->shipping_address_street) ? $account->name . ' (' . $account->shipping_address_street . ')' : $account->name;
        } else if ($this->account_id_c) {
            $account = BeanFactory::getBean('Accounts', $this->account_id_c);
            $return_array['ACCOUNT_C'] = ($account->shipping_address_street) ? $account->name . ' (' . $account->shipping_address_street . ')' : $account->name;
        } else if ($this->ci_customeritems_accountsaccounts_ida) {
            $account = BeanFactory::getBean('Accounts', $this->ci_customeritems_accountsaccounts_ida);
            $return_array['CI_CUSTOMERITEMS_ACCOUNTS_NAME'] = ($account->shipping_address_street) ? $account->name . ' (' . $account->shipping_address_street . ')' : $account->name;
        } else if ($this->accounts_rrq_regulatoryrequests_1accounts_ida) {
            $account = BeanFactory::getBean('Accounts', $this->accounts_rrq_regulatoryrequests_1accounts_ida);
            $return_array['ACCOUNTS_RRQ_REGULATORYREQUESTS_1_NAME'] = ($account->shipping_address_street) ? $account->name . ' (' . $account->shipping_address_street . ')' : $account->name;
        } else if (! $this->fieldDefs['parent_type'] || $this->fieldDefs['parent_type']['value'] == 'Accounts' && $this->parent_id) {
            $account = BeanFactory::getBean('Accounts', $this->parent_id);
            $return_array['PARENT_NAME'] = ($account->shipping_address_street) ? $account->name . ' (' . $account->shipping_address_street . ')' : $account->name;
        }
        // APX Custom Codes: Append street address on account relate fields on both edit and detail views -- END
