...
$titles=array(	
				'module'=>$mod_strings['LBL_SETTINGS_QUICKCRM_TITLE'],
				'fields'=>$MBmod_strings['LBL_EDITVIEW'],
...
function AddProfile($module,$profile_id,$profile_name,$node=false){
	...
	if ($profile_id != '_default'){
	    ...
	    $nodeSettings =new Node($module."Settings", $titles['module']);
    	$nodeSettings->set_property("href", "javascript: checkModuleType('$module','module','$profile_id','$profile_name')");
    	$node->add_node($nodeSettings);       
	...
	$nodeQSearch =new Node($module."PopupSearch", $titles['popupsearch']);
    $nodeQSearch->set_property("href", "javascript: checkModuleType('$module','popupsearch','$profile_id','$profile_name')");
    $node->add_node($nodeQSearch);       
