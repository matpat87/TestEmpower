...
class AOS_InvoicesViewDetail extends ViewDetail {
	...
	function display(){
        ...
        //OnTrack #808 - Link to Users
        if(!empty($this->bean->users_aos_invoices_1users_ida)){
            $this->bean->users_aos_invoices_1_name = '<a href="./index.php?module=Users&action=DetailView&record='. $this->bean->users_aos_invoices_1users_ida .'">'. $this->bean->users_aos_invoices_1_name .'</a>';
        }

		$orderBean = BeanFactory::getBean('ODR_SalesOrders')->retrieve_by_string_fields(
			array(
				"number" => $this->bean->order_number_c,
			), false, true
		);

		$orderURL = ($orderBean && $orderBean->id)
			? "<a href='{$sugar_config['site_url']}/index.php?module=ODR_SalesOrders&action=DetailView&record={$orderBean->id}'>{$this->bean->order_number_c}</a>"
			: $this->bean->order_number_c;

		$this->ss->assign("ORDER_URL", $orderURL);

        echo '<link rel="stylesheet" href="custom/modules/AOS_Invoices/css/detailview.css"/>';
		parent::display();
        echo '<script type="text/javascript" src="custom/modules/AOS_Invoices/js/detailview.js"></script>';
	}
