...
/* APX Custom Codes: Added codes for ReCaptcha and Honeypot -- START */
/* $formSel input[type="submit"] {display: inline; float: none; padding: 5px 10px; width: auto; min-width: auto;} */
$formSel input[type="submit"] {display: inline; float: none; padding: 5px 10px; width: auto; min-width: auto; cursor: pointer}
/* APX Custom Codes: Added codes for ReCaptcha and Honeypot -- END */
...
/* APX Custom Codes: Added codes for ReCaptcha and Honeypot -- START */
$formSel .g-recaptcha div:first-child { margin: 15px auto !important }
$formSel input[type="submit"].disabled { pointer-events: none; }
$formSel .wrapper { cursor: not-allowed; }
$formSel input[name="alt_email_address"] { position: absolute; left: -2000px; width: 5% }
/* APX Custom Codes: Added codes for ReCaptcha and Honeypot -- END */
</style>
<!-- TODO ???
...
HTML;
    ...
    private static function getFormFooterHTML($webFormFooter, $webFormSubmitLabel, $webFormCampaign, $webRedirectURL, $webAssignedUser, $booleanFields, $moduleDir)
    {
        // APX Custom Codes: Added codes for ReCaptcha and Honeypot -- START
        global $sugar_config;
        $captchaSiteKey = $sugar_config['reCaptchaSiteKey'];
        $honeyPotField = "<input type='text' id='alt_email_address' name='alt_email_address' placeholder='Enter Alt Email Address' autocomplete='on' tabindex='-1' value=''>";
        // APX Custom Codes: Added codes for ReCaptcha and Honeypot -- END
        ...
        $html = <<<HTML
$webFormFooter
$honeyPotField <!-- APX Custom Codes: Added codes for ReCaptcha and Honeypot -->
<div class="row center buttons">
    <!-- APX Custom Codes: Added codes for ReCaptcha and Honeypot -- START -->
    <!-- <input type="submit" onclick="submit_form();" class="button" name="Submit" value="$webFormSubmitLabel" /> -->
    <div class="g-recaptcha" data-callback="recaptchaCallback" data-sitekey="$captchaSiteKey"></div>
    <span class="wrapper">
        <input type="submit" onclick="submit_form();" class="button disabled" name="Submit" value="$webFormSubmitLabel" />
    </span>
    <!-- APX Custom Codes: Added codes for ReCaptcha and Honeypot -- END -->
    <div class="clear">&nbsp;</div>
</div>
...
<script type='text/javascript'>
    ...
    function recaptchaCallback() {
        ...
        document.querySelector('input[type="submit"].disabled').classList.remove('disabled');
    };
    /* APX Custom Codes: Added codes for ReCaptcha and Honeypot -- END */
</script>
HTML;
