...
{if $hideTable == false}
    <div class="list-view-rounded-corners">
        <table cellpadding='0' cellspacing='0' border='0' class='list view table-responsive'>
            ...
            {include file='themes/SuiteP/include/ListView/ListViewPaginationTop.tpl'} {* APX Custom Codes *}

            <tr height='20'>
                {if $prerow}
                    <th class="td_alt" style='border-top-left-radius: 0px;'>&nbsp;</th> {* APX Custom Codes: Added Style *}
                {/if}
                {if !(isset($options.hide_edit_link) && $options.hide_edit_link === true) && !empty($quickViewLinks)}
                    <th class='td_alt quick_view_links'>&nbsp;</th>
                {/if}
                <th>{$pageData.additionalDetails.$id}</th> {* APX Custom Codes *}
                ...
                <th style='border-top-right-radius: 0px;'>{$pageData.additionalDetails.$id}</th> {* APX Custom Codes *}
            </tr>
            {* {include file='themes/SuiteP/include/ListView/ListViewPaginationTop.tpl'} *} {* APX Custom Codes *}
            </thead>
            <tbody>
            ...
            {foreach name=rowIteration from=$data key=id item=rowData}
                ...
                <tr height='20' class='{$_rowColor}S1'>
                    ...
                    {if !empty($quickViewLinks)}
                        ...
                        {else}
                            <td>
                                {* APX CUSTOM CODE: CUSTOM_CAN_EDIT is assigned in CustomRRQ_RegulatoryRequestListViewSmarty *}
                                {if $pageData.rowAccess[$id].edit && !empty($quickViewLinks) && $rowData.CUSTOM_CAN_EDIT == true}
                        ...
                        <td align='right'>{$pageData.additionalDetails.$id}</td> {* APX Custom Codes *}
                    {/if}
                    {counter start=0 name="colCounter" print=false assign="colCounter"}
                    ...
                    {/foreach}
                    <td>&nbsp;</td> {* APX Custom Codes - Add blank td to force table row to cover all gaps*}
                </tr>
                {foreachelse}
