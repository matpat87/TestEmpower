...
class SugarEmailAddress extends SugarBean
{
    ...
    public function AddUpdateEmailAddress($addr, $invalid = 0, $opt_out = 0, $id = null, $optInFlag = null)
    {
        ...
        $q = "SELECT * FROM email_addresses WHERE email_address_caps = '{$addressCaps}' and deleted=0 LIMIT 1"; // APX Custom Codes: Fixed issue by adding LIMIT in query as it causes white screen issue if all email addresses are the same and user hits edit and save on any module with an email field (Ex. Users, Contacts, etc)
        $r = $this->db->query($q);
