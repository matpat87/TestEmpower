...
<div class='listViewBody'>
    ...
    {* APX Custom Codes: Change Form Method to GET if Module is not Report -- START *}
    {if $module|strstr:"Report"}
        {assign var="requestMethod" value="POST"}
    {else}
        {assign var="requestMethod" value="GET"}
    {/if}

    <form name='search_form' id='search_form' class='search_form {if !$searchFormInPopup} non-popup{/if}' method='{$requestMethod}'
          action='index.php?module={$module}&action={$action}' onkeydown='submitOnEnter(event);'>
    {* APX Custom Codes: Change Form Method to GET if Module is not Report -- END *}
