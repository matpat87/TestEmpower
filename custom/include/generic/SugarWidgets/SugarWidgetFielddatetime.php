...
class SugarWidgetFieldDateTime extends SugarWidgetReportField
{
	...
	function get_start_end_date_filter(& $layout_def, $begin,$end)
	{
        ...
        if ($layout_def['name'] == 'birthdate') {
            return "DATE_FORMAT({$field_name},'%m-%d') >= DATE_FORMAT({$this->reporter->db->quoted($begin)},'%m-%d') AND DATE_FORMAT({$field_name},'%m-%d') <= DATE_FORMAT({$this->reporter->db->quoted($end)},'%m-%d')\n";
        }
        
        if ($layout_def['name'] == 'last_activity_date_c') {
            if (!isset($begin) && !isset($end)) {
                return "DATE_FORMAT({$field_name},'%Y-%m-%d') IS NULL OR {$field_name} = '' \n";
            }
            return "DATE_FORMAT({$field_name},'%Y-%m-%d') <= DATE_FORMAT({$this->reporter->db->quoted($begin)},'%Y-%m-%d')\n";
        }
    ...
    function queryFilterTP_no_update($layout_def)
	{
		return $this->get_start_end_date_filter($layout_def, null, null);
    }

    function queryFilterTP_no_update_last_30_days($layout_def)
	{
		global $timedate;
        $begin = $this->now()->get("-30 days")->get_day_begin();
		return $this->get_start_end_date_filter($layout_def,$begin->asDb(), null);
    }
    
    function queryFilterTP_no_update_last_60_days($layout_def)
	{
		global $timedate;
		$begin = $this->now()->get("-60 days")->get_day_begin();
		return $this->get_start_end_date_filter($layout_def,$begin->asDb(), null);
    }
    
    function queryFilterTP_no_update_last_90_days($layout_def)
	{
		global $timedate;
		$begin = $this->now()->get("-90 days")->get_day_begin();
		return $this->get_start_end_date_filter($layout_def,$begin->asDb(), null);
    }
    ...
    function displayInput($layout_def) {
        ...
        } else if ($layout_def['name'] == 'last_activity_date_c') {
            $filterTypes = array(
                'TP_no_update'                => $home_mod_strings['LBL_NO_ACTIVITY'],
                'TP_no_update_last_90_days'  => $home_mod_strings['LBL_GREATER_THAN_90_DAYS'],
                'TP_no_update_last_60_days'  => $home_mod_strings['LBL_GREATER_THAN_60_DAYS'],
                'TP_no_update_last_30_days'  => $home_mod_strings['LBL_GREATER_THAN_30_DAYS'],
            );
        } else {
            $filterTypes = array(
                ' '                => $app_strings['LBL_NONE'],
