...
class EditView
{
    ...
    public function process($checkFormName = false, $formName = '')
    {
        ...
        if ($this->focus) {
            ...
            if ($this->focus->account_id) {
                $account = BeanFactory::getBean('Accounts', $this->focus->account_id);
                $this->fieldDefs['account_name']['value'] = ($account->shipping_address_street) ? $account->name . ' (' . $account->shipping_address_street . ')' : $account->name;
            } else if ($this->focus->account_id_c) {
                $account = BeanFactory::getBean('Accounts', $this->focus->account_id_c);
                $this->fieldDefs['account_c']['value'] = ($account->shipping_address_street) ? $account->name . ' (' . $account->shipping_address_street . ')' : $account->name;
            } else if ($this->focus->ci_customeritems_accountsaccounts_ida) {
                $account = BeanFactory::getBean('Accounts', $this->focus->ci_customeritems_accountsaccounts_ida);
                $this->fieldDefs['ci_customeritems_accounts_name']['value'] = ($account->shipping_address_street) ? $account->name . ' (' . $account->shipping_address_street . ')' : $account->name;
            } else if ($this->focus->accounts_rrq_regulatoryrequests_1accounts_ida) {
                $account = BeanFactory::getBean('Accounts', $this->focus->accounts_rrq_regulatoryrequests_1accounts_ida);
                $this->fieldDefs['accounts_rrq_regulatoryrequests_1_name']['value'] = ($account->shipping_address_street) ? $account->name . ' (' . $account->shipping_address_street . ')' : $account->name;
            } else if (! $this->fieldDefs['parent_type'] || $this->fieldDefs['parent_type']['value'] == 'Accounts' && $this->focus->parent_id) {
                $account = BeanFactory::getBean('Accounts', $this->focus->parent_id);
                $this->fieldDefs['parent_name']['value'] = ($account->shipping_address_street) ? $account->name . ' (' . $account->shipping_address_street . ')' : $account->name;
            } else if (! $this->fieldDefs['parent_type'] || $this->fieldDefs['parent_type']['value'] == 'Accounts' && ! $this->focus->parent_id && $this->fieldDefs['parent_id']['value']) {
                $account = BeanFactory::getBean('Accounts', $this->fieldDefs['parent_id']['value']);
                $this->fieldDefs['parent_name']['value'] = ($account->shipping_address_street) ? $account->name . ' (' . $account->shipping_address_street . ')' : $account->name;
            }
