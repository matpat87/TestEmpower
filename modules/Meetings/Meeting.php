...
class Meeting extends SugarBean
{
    ...
    // APX Custom Codes: OnTrack #1236 -- START
    //OnTrack #1236 
        ...
        $singleSelect = false, $ifListForExport = false)
    {
		...
		$result = parent::create_new_list_query($order_by, $where, $filter, $params, 
            ...
            $parent_type_query = $this->get_parent_type_query();
        
        if ((isset($_REQUEST['module']) && $_REQUEST['module'] == 'Meetings')
            && !empty($_REQUEST) && $_REQUEST['action'] == 'index'){
                $result['from_min'] .= ' ' . $parent_type_query['left_join'];
                $result['from'] .= ' ' . $parent_type_query['left_join'];
                $result['select'] .= ' ,' . $parent_type_query['select'];
        }

        //OnTrack #1309
        if(isset($_REQUEST['entryPoint']) && $_REQUEST['entryPoint'] == 'export'){
            $result = str_replace('where', $parent_type_query['left_join'] . ' where', $result);
            $result = str_replace('from', $parent_type_query['select'] . ' from', $result);
        }
    ...
    //OnTrack #1236 
    private function get_parent_type_query(){
        global $app_list_strings;
        $result = array('left_join' => '', 'select' => '', 'count' => 0);

        $parent_type_alias = 'parent_type';
        $sql = "";
        $i = 1;
        $result['select'] = "CASE ";
        foreach($app_list_strings['parent_type_display'] as $parent_type => $parent_type_val){
            ++$i;

            if(!empty($parent_type)){
                $parent_type_temp = strtolower($parent_type);
                $parent_table = $this->get_parent_table_name($parent_type_temp);
                $parent_name = $this->get_parent_name($parent_type_temp, $parent_type_alias . $i);

                $result['left_join'] .= "left join {$parent_table} {$parent_type_alias}{$i} on {$parent_type_alias}{$i}.id = meetings.parent_id ";
                $result['select'] .= "WHEN {$parent_name} IS NOT NULL THEN {$parent_name} ";
            }

            
        }

        $result['select'] .= "END parent_name ";
        $result['count'] = $i;
    ...
    //OnTrack #1236
    private function get_parent_table_name($parent_name_param){
        $result = $parent_name_param;

        //manual stuffs
        if($parent_name_param == 'projecttask'){
            $result = "project_task";
        }
    ...
    //OnTrack #1236
    private function get_parent_name($parent_name_param, $parent_type_alias){
        $result = $parent_type_alias . ".name";
        $manual_parent_name = array('contacts', 'leads', 'prospects', 'vc_vendorcontacts');

         //manual stuffs again
        if(in_array($parent_name_param, $manual_parent_name)){
            $result = "concat({$parent_type_alias}.first_name, {$parent_type_alias}.last_name) ";
        }

        return $result;
    }
    
    // APX Custom Codes: OnTrack #1236 -- END
    ...
    public function fill_in_additional_detail_fields()
    {
        ...
        if (isset($_REQUEST['parent_type']) && empty($this->parent_type)) {
            ...
            // APX Custom Codes: OnTrack #177 - Set "Related To" field to required and set default value to blank
    ...
    public function get_list_view_data()
    {
        ...
        // APX Custom Codes: Updated account relate fields to display street address in list view -- START
        // $meeting_fields['PARENT_NAME'] = $this->parent_name;
        if ($this->parent_type !== 'Accounts') {
            $meeting_fields['PARENT_NAME'] = $this->parent_name;
        }
        // APX Custom Codes: Updated account relate fields to display street address in list view -- END
        ...
        // APX CUstom Codes: Modified to handle Status column in subpanel -- START
        if ($_REQUEST['module'] !== 'Meetings' && ($_REQUEST['action'] == 'DetailView' || $_REQUEST['action'] == 'SubPanelViewer')) {
			global $app_list_strings;
			
			$meetingBean = BeanFactory::getBean('Meetings', $meeting_fields['ID']);
			$meetingType = $meetingBean && $meetingBean->meeting_type_c ? $app_list_strings['meeting_type_list'][$meetingBean->meeting_type_c] : '';

			$meeting_fields['STATUS'] = $meetingType ? 
				"{$meeting_fields['STATUS']} - {$meetingType}" : 
				$meeting_fields['STATUS'];
        }
        // APX CUstom Codes: Modified to handle Status column in subpanel -- END
