...
class OutcomeByMonthDashlet extends DashletGenericChart
{
    ...
    public $obm_users = array(); // APX Custom Codes
    public $obm_date_start;
    public $obm_date_end;
    public $obm_sales_stages = array(); // APX Custom Codes
    ...
    public function displayOptions()
    {
        ...
        if (count($this->obm_sales_stages) > 0)
            foreach ($this->obm_sales_stages as $key) {
                $selected_datax[] = $key;
            }
        else {
        ...
        if (!isset($this->obm_ids) || count($this->obm_ids) == 0) {
            ...
            $this->_searchFields['obm_users']['options'] = getUserRepresentativesForReports(); // APX Custom Codes
            $this->_searchFields['obm_sales_stages']['options'] = $app_list_strings['sales_stage_dom']; // APX Custom Codes
            $this->_searchFields['obm_sales_stages']['input_name0'] = $selected_datax; // APX Custom Codes
        }
...
EOD;
    ...
    protected function constructQuery()
    {
        ...
        $query = "SELECT sales_stage, ".DBManagerFactory::getInstance()->convert('date_closed','date_format',array("'%Y-%m'"),array("'YYYY-MM'"))." as m, SUM(amount / 1000) AS total, COUNT(*) AS opp_count
            FROM (
                ...
                WHERE opportunities.deleted = 0
                    AND opportunities.date_closed >= ". DBManagerFactory::getInstance()->convert("'".$this->obm_date_start."'",'date'). " 
                    AND opportunities.date_closed <= ".DBManagerFactory::getInstance()->convert("'".$this->obm_date_end."'",'date');
           

            if ( isset($this->obm_users) && count($this->obm_users) > 0 ) {
//                $query .= " AND securitygroups_users.user_id IN ('" . implode("','",$this->obm_users) . "') ";
                $query .= " AND opportunities.assigned_user_id IN ('" . implode("','",$this->obm_users) . "') ";
            } else {
                $query .=  (! $current_user->is_admin) ? "AND securitygroups_users.user_id = '{$current_user->id}'" : '';
            }

            if ( isset($this->obm_sales_stages) && count($this->obm_sales_stages) > 0 ) {
                $query .= " AND opportunities.sales_stage IN ('" . implode("','", $this->obm_sales_stages) . "') ";
            }

            $query .= "  GROUP BY opportunities.id ) AS chartData
            GROUP BY sales_stage , ". DBManagerFactory::getInstance()->convert('date_closed','date_format',array("'%Y-%m'"),array("'YYYY-MM'")) . "
            ORDER BY m";
        // APX Custom Codes -- END
