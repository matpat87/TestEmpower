<?php
	...
	$filters = [
		'my_opportunities' => $_REQUEST['opportunity_by_volume_my_opportunities'] ?? [],
		'sales_stage' => $_REQUEST['opportunity_by_volume_sales_stages'] ?? [],
		'assigned_to' => $_REQUEST['opportunity_by_volume_sales_group_user_ids'] ?? [],
		'amount' => $_REQUEST['opportunity_by_volume_amount'] ?? '',
		'date_from' => $_REQUEST['opportunity_by_volume_date_from'] ?? '',
		'date_to' => $_REQUEST['opportunity_by_volume_date_to'] ?? '',
	];
	...
	foreach ($salesStages as $key => $value) {
		...
		$query .= " 
			(
				SELECT IFNULL(COUNT(opportunities.sales_stage), 0) FROM opportunities
