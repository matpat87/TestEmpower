...
EOQ;
    ...
    public function mass_assign($event, $arguments)
    {
        ...
        $no_mass_assign_list = array("ACLRoles"=>"ACLRoles");
            ...
            if (is_admin($current_user) || ACLAction::getUserAccessLevel($current_user->id, "SecurityGroups", 'access') == ACL_ALLOW_ENABLED) {
                ...
                if (array_key_exists($module, $security_modules)) {
                    ...
                    foreach ($groups['list'] as $group) {
                        ...
                        $type = explode(" ", ucfirst($GLOBALS['app_list_strings']['sec_group_type_dom'][$group->type_c]));
                        $acronym = "";
                        ...
                        switch ($group->type_c) {
                            case 'SAMAccountAccess':
                                $acronym = 'SAM';
                                break;
                            case 'MDMAccountAccess':
                                $acronym = 'MDM';
                                break;
                            default:
                                $acronym;
                        ...
                        $options[$group->id] = $acronym ? "{$acronym} - {$group->name}" : $group->name;
                        // APX Custom Codes -- END
                    }
