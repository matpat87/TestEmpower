...
{if $hideTable == false}
	...
	<thead>
		...
		<tr height='35'>
			{if $prerow}
				<th class="td_alt" style='border-top-left-radius: 0px;'>&nbsp;</th>
			{/if}
			...
			<th style='border-top-right-radius: 0px;'>{$pageData.additionalDetails.$id}</th>
		</tr>
	</thead>
	<tbody>
			...
			<tr height='35' class='{$_rowColor}S1'>
				...
				<td>
				 {if !$is_admin && $is_admin_for_user && $rowData.IS_ADMIN==1}
						<input type='checkbox' disabled="disabled" class='listview-checkbox' value='{$rowData.DISTRO_ITEM_ID_NON_DB}'>
				 {else}
	                    <input title="{sugar_translate label='LBL_SELECT_THIS_ROW_TITLE'}" onclick='sListView.check_item(this, document.MassUpdate)' type='checkbox' class='listview-checkbox' name='mass[]' value='{$rowData.DISTRO_ITEM_ID_NON_DB}'>
				 {/if}
				</td>
