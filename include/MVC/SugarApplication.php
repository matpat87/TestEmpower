...
class SugarApplication
{
    ...
    public function loadUser()
    {
        ...
        $allowed_actions = (!empty($this->controller->allowed_actions)) ? $this->controller->allowed_actions : $allowed_actions = array('Authenticate', 'Login', 'SAMLLogin', 'LoggedOut'); // APX Custom Codes: OnTrack #1941 - Added custom SAML Login Page
        ...
        if (isset($_SESSION['authenticated_user_id'])) {
            ...
            if (!$authController->sessionAuthenticate()) {
                ...
                // APX Custom Codes: OnTrack #1941 - Added custom SAML Login Page -- START
                (isset($sugar_config['authenticationClass']) && $sugar_config['authenticationClass'] == 'SAML2Authenticate')
                    ? SugarApplication::redirect('index.php?action=SAMLLogin&module=Users')
                    : SugarApplication::redirect('index.php?action=Login&module=Users');
                // APX Custom Codes: OnTrack #1941 - Added custom SAML Login Page -- END
                die();
            }//fi
