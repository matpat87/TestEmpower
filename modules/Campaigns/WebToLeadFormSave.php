...
// APX Custom Codes: Resolve issue where javascript is not being loaded due to tinyMCE removing Script Tags
$bodyHTML = str_replace(array("// <![CDATA[", "// ]]>"), array("<script type='text/javascript'> // <![CDATA[", "// ]]> </script>"), $bodyHTML);
...
if (stripos((string) $html, "<html") === false) {
    ...
    $googleAPI = "<script src='https://www.google.com/recaptcha/api.js' async defer></script>";
    $html = "<html {$langHeader}><head><meta http-equiv=\"Content-Type\" content=\"text/html; charset=UTF-8\"></head>{$googleAPI}<body>" . $html . "</body></html>";
