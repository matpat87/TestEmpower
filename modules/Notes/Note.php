...
class Note extends File
{
    ...
    public function get_list_view_data()
    {
        ...
        // APX Custom Codes: Ontrack #1579 - Excluded for parent_type = 'Accounts' to display its customized parent_name format
        if (isset($this->parent_name) && $this->parent_type <> 'Accounts') {
            $note_fields['PARENT_NAME'] = $this->parent_name;
        }
    ...
    // APX Custom Codes: Ontrack #1579 - Extended core method: create_new_list_query for sorting 'parent_name' column customizations -- START
    ...
    {
        global $log;
        // Temporarily sets $order_by to 'name' when $_REQUEST['Notes2_NOTE_ORDER_BY'] so on trigger
        // parent::create_new_list_query will not result to a query error
        $order_by = isset($_REQUEST['Notes2_NOTE_ORDER_BY']) && $_REQUEST['Notes2_NOTE_ORDER_BY'] == 'parent_name' ? 'name' : $order_by; 
        ...
        /**
         * On Sort Related To in List View
         * Trigger custom method, customParentNameQuery() to append dynamic parent_name columns and left join tables based on the column <parent_type> value
         * This way we can use ORDER BY 'parent_name' in this query without errors
         */
        if (isset($_REQUEST['Notes2_NOTE_ORDER_BY']) && $_REQUEST['Notes2_NOTE_ORDER_BY'] == 'parent_name') {

            $customSelect = $this->customParentNameQuery();
            $result['select'] = $result['select'] . $customSelect['select'];
            $result['from_min'] .= ' ' . $customSelect['left_join'];
            $result['from'] .= ' ' . $customSelect['left_join'];
            $result['order_by'] = " ORDER BY parent_name {$_REQUEST['lvso']}";
    ...
    /**
     * For Ontrack #1579: To fix Sorting Related To (parent_name) Column
     * Used for Sorting Related To (parent_name) Column in List View
     * Appends a CASE WHEN in $result['select'] from create_new_list_query; Case when <parent_type> THEN <parent_type>.name AS 'parent_name'
     * Added Condition for parent types that has <first_name><space><last_name> as parent names specified in an array of table_names: $customParentNameModules with such modules ex (leads, contacts)
     * Appends an added LEFT JOIN blocks for every parent_types present
     * @return Array $result
     */
    private function customParentNameQuery()
    {
        global $db, $log;
        
        $appendSelectString = "CASE ";
        $result = array('left_join' => '', 'select' => ' ,' );
        $customParentNameModules = array('contacts', 'leads', 'prospects', 'vc_vendorcontacts');

        $parentTypesSql = $db->query("SELECT DISTINCT(parent_type) FROM notes WHERE  notes.deleted = 0"); // Retrieve all parent_types existing in db table

        while ($row =  $db->fetchByAssoc($parentTypesSql)) {
            $tableName = !empty($row['parent_type']) ? strtolower($row['parent_type']) : '';

            if ($tableName != "") {
                $result['left_join'] .= "LEFT JOIN {$tableName} ON {$tableName}.id = notes.parent_id ";
                if (!in_array($tableName, $customParentNameModules)) {
                    $appendSelectString .= "WHEN notes.parent_type = '{$row['parent_type']}' THEN {$tableName}.name ";

                } else {
                    $appendSelectString .= "WHEN notes.parent_type = '{$row['parent_type']}' THEN CONCAT({$tableName}.first_name,' ',{$tableName}.last_name) ";
                }
            }
        }

        $appendSelectString .= " ELSE '' END parent_name ";
        $result['select'] .= $appendSelectString;
        
        return $result;
    }
    // APX Custom Codes: Ontrack #1579 - Extended core method: create_new_list_query for sorting 'parent_name' column customizations -- END
}
