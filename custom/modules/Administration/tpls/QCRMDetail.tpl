{literal}
<style>
	#conf_sortableD1, #conf_sortableD2, #conf_sortableD3 { border-color: black;border-style: solid; border-width:1px; text-align:center ;margin:10px; width: 95%;list-style-type: none; padding: 0 0 2.5em; float: left; margin-right: 10px; }
	#conf_sortableD1 li, #conf_sortableD2 li, #conf_sortableD3 li { margin: 0 5px 5px 5px; padding: 5px; font-size: 1.2em; width: initial; text-align:left ;}
 	#conf_sortableD2 li span.ui-icon{display: none;}
 	#conf_sortableD1 li span.ui-icon{position: absolute;}
 	#conf_sortableD1 li {text-align:center}
    .fieldsColumn{
        width: 38%;
		max-width: 380px;
		float:left;
...
{if $SHOWCOPYSTUDIO}
	<input title='{$COPYSTUDIO}' accessKey='M' class='button' onclick="if (confirm('{$MOD.LBL_STUDIO_QUICKCRM_CONFIRMATION}')) {ldelim}CopyFromStudio('{$module}','detail','{$profile}'); return true;{rdelim} return false;" type='button' name='button' value='{$COPYSTUDIO}'></input>
{/if}
</div>
...
<div id="confmodule" {$HIDEFIELDS}>
	<div class="fieldsColumn">
		<div id="hidden_div">
			<h3></h3>
			<input type="text" id="search_field_sortable2" name="search" placeholder="{$APP_STRINGS.LBL_SEARCH}" class="searchInput">
			<ul id="conf_sortableD2" class="connectedSortable">{$HIDDEN}
{foreach name=tabCom from=$tabHidden key=tabColkey item=tabHiddenData}
                <li id= 'q_{$tabHiddenData.field}' class='ui-state-default'><span class="ui-icon ui-icon-arrowthick-2-n-s"></span>
...
{/foreach}
	...
	<div class="fieldsColumn">
		<div id="selected_div">
			<h3></h3>
			<input type="text" id="search_field_sortable1" name="search" placeholder="{$APP_STRINGS.LBL_SEARCH}" class="searchInput">
...
{foreach name=tabAvailable from=$tabAvailable key=tabColkey item=tabAvailableData}
			{if $tabAvailableData.element == 'field'}
                <li id= 'q_{$tabAvailableData.field}' class='ui-state-highlight'><span class="ui-icon ui-icon-arrowthick-2-n-s"></span>
        ...
        <div id="panel_div">
            ...
            <ul id="conf_sortableD3" class="connectedSortable">{$NEWPANEL}
                {foreach name=tabCom from=$tabTools key=tabColkey item=tabToolsData}
                    <li id= 'p_{$tabToolsData.field}' class='ui-state-default label_initial'>
