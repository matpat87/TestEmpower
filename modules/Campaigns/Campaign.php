...
class Campaign extends SugarBean
{
    ...
    */
    ...
    public function track_log_entries($type=array())
    {
        ...
        // $query_array['select'] ="SELECT campaign_log.* ";
        $query_array['select'] ="SELECT DISTINCT(campaign_log.target_id), campaign_log.* ";

        if (in_array($type[0], ['targeted', 'invalid email', 'send error', 'blocked'])) {
            $query_array['select'] .= ", COUNT(campaign_log.hits) AS hits ";
        }
        ...
        // APX Custom Codes: Fix Campaign Log Subpanel Count -- START
        /* No longer needed as counters were fixed with customizations made on the main query (Ex. Distinct, Group By handling, SugarBean changes)
        //B.F. #37943
        ...
        */
        ...
        // APX Custom Codes: $type[1] is set on _override_custom_campaign_log_subpanels
        if (isset($type[1])) {
            // APX Custom Codes: Added Query to retrieve data for Not Viewed Message and Not Click-Thru Link Subpanels -- START
            $customSelect = ", NULL AS related_id, NULL AS related_type";
            $customSelect .= ($type[1] <> 'link')
                ? ", 'Not Viewed Message' AS activity_type"
                : ", 'Not Click-thru Link' AS activity_type";

            $query_array['select'] .= " {$customSelect} ";

            $customWhere = " AND campaign_log.target_id NOT IN ( ";
            $customWhere .= " SELECT campaign_log.target_id {$query_array['from']} {$query_array['where']} ";
            $customWhere .= " ) ";
            $customWhere = str_replace("AND activity_type='{$type[0]}'", "AND activity_type='{$type[1]}'", $customWhere);
            ...
            $query_array['where'] .= "{$customWhere} GROUP BY campaign_log.target_id HAVING {$count}";
            // APX Custom Codes: Added Query to retrieve data for Not Viewed Message and Not Click-Thru Link Subpanels -- END
        } else {
            $query_array['where'] .= " GROUP BY campaign_log.target_id "; // APX Custom Codes: Fix Campaign Log Subpanel Count
        }
...
//
    ...
    public function create_list_count_query($query, $params=array())
    {
        ...
        // APX Custom Codes: Fix Campaign Log Subpanel Count -- START
        if ((strpos($query, 'DISTINCT(campaign_log.target_id)'))) {
            $this->seed->table_name = 'campaign_log';
        }
        // APX Custom Codes: Fix Campaign Log Subpanel Count -- END
