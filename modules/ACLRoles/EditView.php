...
foreach ($app_list_strings['user_division_list'] as $key => $value) {
	if ($key == $role->division) {
		$roleDivisionOptions .= "<option value='{$key}' selected='selected'>{$value}</option>";
	} else {
		$roleDivisionOptions .= "<option value='{$key}'>{$value}</option>";
	}
}
...
$sugar_smarty->assign('DIVISION_OPTIONS', $roleDivisionOptions); // APX Custom Codes: OnTrack #607 - Added Division Field
