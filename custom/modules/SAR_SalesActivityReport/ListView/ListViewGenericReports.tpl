...
{include file='custom/modules/SAR_SalesActivityReport/inlcudes/management_update_modal.tpl'}
<script type='text/javascript' src='{sugar_getjspath file='include/javascript/popup_helper.js'}'></script>
<script type='text/javascript' src='{sugar_getjspath file='custom/modules/SAR_SalesActivityReport/js/listview.js'}'></script>
...
{if $hideTable == false}
    ...
    <thead>
        ...
        <tr>
            <th colspan="{$colCount}" class="thcstm">Sales Activity Report for {$current_user_name}</th>
        <tr>
        <tr height='35' style="display: block; padding: 0px;">
            {if $prerow}
                <th class="td_alt">&nbsp;</th>
                <th class="td_alt">&nbsp;</th>
            {/if}
            ...
            {foreach from=$displayColumns key=colHeader item=params}
                ...
                {elseif $colHeader == 'SHIPPING_ADDRESS_CITY_C' || $colHeader == 'SHIPPING_ADDRESS_STATE_C' }
                    {assign var='width' value="7.5"}
                {elseif $colHeader == 'RELATED_TO_C' }
                    {assign var='width' value="15"}
                {else}
                    {assign var='width' value="10"}
                {/if}

                {if $colHeader == 'NAME' || $params.bold}
                    <th scope='col' data-toggle="true" class="{$hide}" style="width: {$width}%">
                {else}
                    <th scope='col' data-breakpoints="{$datahide}" class="{$hide}" style="width: {$width}%">
                {/if}
                        <div>
    ...
    <tbody style="display: block; overflow-y: scroll; max-height: 500px;">
        ...
        {foreach name=rowIteration from=$data key=id item=rowData}
            ...
            {/if}
            <tr height='35' class='{$_rowColor}S1'>
                {if $prerow}
                <td style="width: 1%">
                 ...
                 {else}
                        {if $rowData.HIGHLIGHTS_C}
                            <input title="{sugar_translate label='LBL_SELECT_THIS_ROW_TITLE'}" onclick='sListView.check_item(this, document.MassUpdate)' type='checkbox' class='listview-checkbox highlight' name='mass[]' value='{$rowData.ID}' activity_name='{$rowData.ACTIVITY_NAME_C}'>
                        {else}
                            <input title="{sugar_translate label='LBL_SELECT_THIS_ROW_TITLE'}" onclick='sListView.check_item(this, document.MassUpdate)' type='checkbox' class='listview-checkbox' name='mass[]' value='{$rowData.ID}' activity_name='{$rowData.ACTIVITY_NAME_C}'>
                        {/if}
                 {/if}
                </td>
                <td style="width: 1%">
                    {if $show_management_button}
                        <a title="Add/Edit Management Update" 
                            class="management-update-btn"
                            data-id='{$rowData.ID}' 
                            activity-name='{$rowData.ACTIVITY_NAME_C}' href="#"
                            activity-subject='{$rowData.NAME}' href="#"
                            activity-status='{$rowData.STATUS_C}' href="#">
                        
                            {if empty($rowData.MANAGEMENT_UPDATE)}
                ...
                {foreach from=$displayColumns key=col item=params}
                    ...
                    {if $col  == 'NAME' }
                        {assign var='width' value="29"}
                    {elseif $col == 'SHIPPING_ADDRESS_CITY_C' || $col == 'SHIPPING_ADDRESS_STATE_C' }
                        {assign var='width' value="7.5"}
                    {elseif $col == 'RELATED_TO_C' }
                        {assign var='width' value="14"}
                    {else}
                        {assign var='width' value="10"}
                    {/if}

                    <td style="width: {$width}%"{if $scope_row} scope='row' {/if} align='{$params.align|default:'left'}' valign="top" type="{$displayColumns.$col.type}" field="{$col|lower}" class="{$hide} {if $inline_edit && ($displayColumns.$col.inline_edit == 1 || !isset($displayColumns.$col.inline_edit))}inlineEdit{/if}{if ($params.type == 'teamset')}nowrap{/if}{if preg_match('/PHONE/', $col)} phone{/if}">
                ...
                <tr height='35' class='{$_rowColor}S1'>
                    <td></td>
                    <td></td>
                    <td colspan="8" style="max-width: 300px !important; word-wrap:break-word;">{sugar_field parentFieldArray=$rowData vardef=$params displayType=ListView field='description'}</td>
                </tr>
                {/if}
                {if !empty($data[$rowDataCount].MANAGEMENT_UPDATE)}
                <tr style="background-color: white">
                    <td></td>
                    <td></td>
                    <td colspan="8" style="max-width: 300px !important; word-wrap:break-word;">
                        <p>****Management Update****</p>
                        {sugar_field parentFieldArray=$rowData vardef=$params displayType=ListView field='management_update'}
                    </td>
                </tr>
                {/if}
        ...
        {foreachelse}
        <tr height='35' class='{$rowColor[0]}S1'>
            <td colspan="{$colCount}">
