...
$chart = <<<EOD
        ...
        <script>
            ...
            function resizeGraph(graph)
            {
              ...
              // APX Custom Codes: Hotfix on page load, rGraph javascript function to properly load the calendar dashlet -- START
              if (graph.__object__) {
              ...
              // APX Custom Codes: Hotfix on page load, rGraph javascript function to properly load the calendar dashlet -- END
            }
            ...
            if(bar[0]!== undefined && bar[0]['id']!==undefined)
            {
                ...
                // APX Custom Codes: Added support for multiple assigned users as parameter on click event from chart -- START
                var assignedUserIds = [];
                $(divHolder).find(".assignedUserIds").each(function() {
                    assignedUserIds.push($(this).val());
                });
                
                if (Array.isArray(assignedUserIds)) {
                    assignedUserIds = assignedUserIds.map(encodeURIComponent).join('&assigned_user_id_advanced[]='); // Convert array to query string
                } else {
                    assignedUserIds = encodeURIComponent(assignedUserIds); // Ensure single value is encoded
                }
                
                // APX Custom Codes: Added support for multiple assigned users as parameter on click event from chart -- END
                
                var labels = bar["object"]["properties"]["chart.labels"];
                var keys = window["chartHBarKeys"+graphId];
                var clicked = encodeURI(keys[bar[5]]);

                window.open('index.php?module='+module+'&action='+action+'&query='+query+'&searchFormTab='+searchFormTab+'&start_range_date_closed_advanced='+startDate+'&assigned_user_id_advanced[]='+assignedUserIds+'&end_range_date_closed_advanced='+endDate+'&date_closed_advanced_range_choice=between'+'&sales_stage='+clicked,'_self');
            }
                else
                alert("Sorry, there has been an error with the click-through event");
            }
