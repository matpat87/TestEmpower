{literal}
<style>
	#conf_sortableD1, #conf_sortableD2 { border-color: black;border-style: solid; border-width:1px; text-align:center ;margin:10px; width: 280px;list-style-type: none; margin: 10px; padding: 0 0 2.5em; float: left; margin-right: 10px; }
	#conf_sortableD1 li, #conf_sortableD2 li { margin: 0 5px 5px 5px; padding: 5px; font-size: 1.2em; width: 260px; text-align:left ;}
...
<div id="confmodule">
	<div style="float:left;width:300px;">
		<div id="hidden_div">
			<h3></h3>
			<input type="text" id="search_field_sortable2" name="search" placeholder="{$APP_STRINGS.LBL_SEARCH}" style="margin-left:10px;width:270px">
			<ul id="conf_sortableD2" class="connectedSortable">{$HIDDEN}
{foreach name=tabCom from=$tabHidden key=tabColkey item=tabHiddenData}
					<li id= '{$tabHiddenData.field}' class='ui-state-default'><span class="ui-icon ui-icon-arrowthick-2-n-s"></span>{$tabHiddenData.label}</li>
{/foreach}
			</ul>
		</div>
	</div>
	<div style="float:left;width:300px;">
		<div id="selected_div">
			<h3></h3>
			<input type="text" id="search_field_sortable1" name="search" placeholder="{$APP_STRINGS.LBL_SEARCH}" style="margin-left:10px;width:270px">
...
{foreach name=tabAvailable from=$tabAvailable key=tabColkey item=tabAvailableData}
					<li id= '{$tabAvailableData.field}' class='ui-state-highlight'><span class="ui-icon ui-icon-arrowthick-2-n-s"></span>{$tabAvailableData.label}</li>
{/foreach}
			</ul>
		</div>
	</div>
