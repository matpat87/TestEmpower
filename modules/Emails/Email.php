...
class Email extends Basic
{
        ...
        if (!empty($request['documents'])) {
            ...
            foreach ($exDocs as $docId) {
                ...
                if (!empty($docId)) {
                    ...
                    $fileLocation = "{$sugar_config['upload_dir']}{$docGUID}"; // APX Custom Codes: Ontrack #1833 - Document storage configuration options
                    ...
                    if ((($this->type == 'draft') && !empty($this->id)) || (isset($request['saveToSugar']) && $request['saveToSugar'] == 1)) {
                        ...
                        $dest = "{$sugar_config['upload_dir']}{$note->id}"; // APX Custom Codes: Ontrack #1833 - Document storage configuration options
                        if (!copy($fileLocation, $dest)) {
            ...
            foreach ($exNotes as $noteId) {
                ...
                if (!empty($noteId)) {
                    ...
                    if (!empty($note->id)) {
                        ...
                        $fileLocation = "{$sugar_config['upload_dir']}{$noteGUID}"; // APX Custom Codes: Ontrack #1833 - Document storage configuration options
    ...
    public function create_new_list_query(
        ...
        // APX Custom Codes: OnTrack #1578 -- START
        /**
         * OnTrack #1578 -- Bug Fix on Column Sort on list view
         * Cusomize $result from create_new_list_query and override static email list $order_by value of 'id' (email id)
         * To check $_REQUEST data Emails2_EMAIL_ORDER_BY, supplied when column sort is clicked
         */
        $result['from'] .= ' LEFT JOIN emails_text et ON et.email_id = emails.id '; // To add from_addr in SELECT query and fix the "From" column sort
        if (isset($_REQUEST['Emails2_EMAIL_ORDER_BY']) && isset($_REQUEST['lvso'])) {
            $result['select'] .= ' , et.from_addr ';  // Include email address 'From' Column in query string
            
            switch ($_REQUEST['Emails2_EMAIL_ORDER_BY']) {
                case 'date_entered':
                case 'assigned_user_name':
                case 'parent_type':
                    $result['order_by'] = " ORDER BY {$_REQUEST['Emails2_EMAIL_ORDER_BY']} {$_REQUEST['lvso']}";
                    break;
                case 'subject':
                    $result['order_by'] = " ORDER BY emails.name {$_REQUEST['lvso']}";
                    break;
                case 'from_addr_name':
                    $columnName = 'et.from_addr';
                    $result['order_by'] = " ORDER BY {$columnName} {$_REQUEST['lvso']}";
                    break;
                case 'assigned_user_name':
                    $columnName = 'assigned_user_name';
                    $result['order_by'] = " ORDER BY {$columnName} {$_REQUEST['lvso']}";
                    break;
            }
        }
        ...
        // APX Custom Codes: OnTrack #1578 -- END
    ...
    public function create_export_query($order_by, $where)
    {
        ...
        if ($contact_required) {
            ...
            // APX Custom Codes: Fixed issue where user can't trigger Security Groups: Mass Assign if via "Select All" -- START
            // Updated export query to include needed columns and tables as core codes doesn't generate the needed relationships based on the parameters passed
            ...
            $query .= " FROM contacts, emails LEFT JOIN emails_contacts ON emails_contacts.contact_id = contacts.id AND emails_contacts.email_id = emails.id LEFT JOIN emails_text ON emails.id = emails_text.email_id";
            $where_auto = "contacts.id = emails.parent_id AND emails.deleted=0 AND contacts.deleted=0";
            // APX Custom Codes: Fixed issue where user can't trigger Security Groups: Mass Assign if via "Select All" -- END
        } else {
            $query = 'SELECT emails.*';
            $query .= $custom_join['select'];
            ...
            // $query .= ' FROM emails ';
            // $where_auto = "emails.deleted=0";
            $query .= ' FROM emails LEFT JOIN emails_text ON emails.id = emails_text.email_id';
            $where_auto = "emails.deleted=0";
            // APX Custom Codes -- END
        }
...
eoq;
    ...
    protected function setupAttachments(
        ...
        global $sugar_config; // APX Custom Codes: Ontrack #1833 - Document storage configuration options
///////////////////////////////////////////////////////////////////////
        ////	ATTACHMENTS
            ...
            foreach ($this->saved_attachments as $note) {
                ...
                if ($note->object_name == 'Note') {
                    if (!empty($note->file->temp_file_location) && is_file($note->file->temp_file_location)) { // brandy-new file upload/attachment
                        ...
                        $file_location = "{$sugar_config['upload_dir']}{$note->id}"; // APX Custom Codes: Ontrack #1833 - Document storage configuration options
