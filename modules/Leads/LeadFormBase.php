    ...
    public function handleSave($prefix, $redirect=true, $useRequired=false, $do_save=true, $exist_lead=null)
    {
        ...
        if (empty($_POST['record']) && empty($_POST['dup_checked'])) {
            ...
            if (isset($duplicateLeads)) {
                ...
                // APX Custom Codes: OnTrack #1303 -- START
                /***
                 * OnTrack # 1303 - Bug fix to avoid copying/or moving all the email history that is tied for an existing email that is already used 
                 * on an existing Lead in the system (duplicate lead)
                 * Example case: when creating a lead that uses the email: empower@chromacolors.com
                 * On Save Lead, it copies/moves all activities/history from other leads that uses the same email to this newly created Lead.
                 * 
                 * $emailAddress = new SugarEmailAddress();
                 * $get .= $emailAddress->getFormBaseURL($focus);
                 * 
                 * */
                // APX Custom Codes: OnTrack #1303 -- END
