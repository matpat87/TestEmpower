...
EOD;
    ...
    protected function constructQuery($datay= array(), $targets=array(), $campaign_id = null, $cache_file_name='a_file', $refresh=false, $marketing_id='', $is_dashlet=false, $dashlet_id='')
    {
        ...
        global $mod_strings, $current_user; // APX Custom Codes
        ...
        $opp_query = "SELECT count(*) opp_count,sum(" . db_convert("opportunities.amount","IFNULL",array(0)).")  total_value
            FROM opportunities
        ...
        WHERE opportunities.deleted = 0
            ...
            AND opportunities.campaign_id = '{$campaign_id}'
            AND opportunities.sales_stage = 'Prospecting'
        ...
        $opp_query1 = "SELECT SUM(opportunities.amount) AS revenue
            FROM opportunities
        LEFT JOIN securitygroups_records 
            ON securitygroups_records.record_id = opportunities.id
            AND securitygroups_records.deleted = 0
