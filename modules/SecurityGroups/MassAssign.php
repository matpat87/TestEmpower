...
require_once('custom/modules/SecurityGroups/helpers/SecurityGroupHelper.php'); // APX Custom Codes for Mass Assigning/Removing Child Accounts Security Group
...
if (isset($_POST['mass']) && is_array($_POST['mass'])) {
    ...
    foreach ($_POST['mass'] as $id) {
        if (isset($_POST['Delete'])) {
            ...
            // APX Custom Codes for Mass Assigning/Removing Child Accounts Security Group -- START
            if ($sugarbean->module_dir !== 'Accounts') {
                $sugarbean->load_relationship($rel_name);
                $sugarbean->$rel_name->delete($sugarbean->id, $groupFocus->id);
            } else {
                $eventLogId = create_guid();
                SecurityGroupHelper::insertOrDeleteAccountChildModuleSecurityGroups('delete', $sugarbean->id, $groupFocus->id, $eventLogId, false);
            }
            // APX Custom Codes for Mass Assigning/Removing Child Accounts Security Group -- END
        ...
        //As of 6.3.0 many-to-many requires a link field set in both modules...so lets bypass that
            ...
            if ($sugarbean->module_dir !== 'Accounts') {
                ...
                $eventLogId = create_guid();
                SecurityGroupHelper::insertOrDeleteAccountChildModuleSecurityGroups('insert', $sugarbean->id, $groupFocus->id, $eventLogId, false);
            }
            // APX Custom Codes for Mass Assigning/Removing Child Accounts Security Group -- END
