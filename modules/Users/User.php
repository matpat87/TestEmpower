...
EOQ;
    ...
    public function setNewPassword($new_password, $system_generated = '0')
    {
        ...
        // APX Custom Codes: OnTrack #644 - Force Password Change -- START
        $query = "UPDATE {$this->table_name}_cstm SET is_reset_password_c = 0 WHERE id_c='{$this->id}' ";
        $this->db->query($query, true, "Error setting new password for $this->user_name: ");
        $GLOBALS['log']->info('Set is_reset_password_c to 0');
        // APX Custom Codes: OnTrack #644 - Force Password Change -- END
    }
    ...
    public function create_new_list_query(
        ...
        /**
         * APX Custom Codes - START for Ontrack #1795 
         * 
         * On User relate field POPUP, it checks for a param in $_REQUEST 'department' OR if the relate field 'users_accounts_3_name'
         * which is the field for the Customer Service user only.
         * This is for relate field Pop-up filtered list, Field Inline search - to search only users in Customer Service, and for Pop-up search
         * @auth Glai Obido
         */
        
        if ($_REQUEST['action'] == 'Popup' || $_REQUEST['action'] == 'quicksearchQuery') {
            
            // This is necessary as the Request params on diff pop-up actions vary
            $requestData = ($_REQUEST['action'] == 'quicksearchQuery') 
                ? $_REQUEST['data']
                : $_REQUEST['request_data'];
            
                
            $requestDataDecoded = json_decode(html_entity_decode($requestData), true);
            
            // To verify if the relate field is 'users_accounts_3_name'
            $keyToCheck = ($_REQUEST['action'] == 'quicksearchQuery') 
                ? in_array('users_accounts_3_name', $requestDataDecoded['populate_list'])
                : $requestDataDecoded['field_to_name_array']['name'] == 'users_accounts_3_name';
            
            if ((isset($_REQUEST['department']) && $_REQUEST['department'] == 'CustomerService') || $keyToCheck) {
                $ret_array['where'] .= " AND users.department='CustomerService' AND users.status='Active'";
            } 
            
            
        }

        if (isset($_REQUEST['reports_to_name_advanced'])) { // Custom string replace id Reports To field is being searched or has value
            $ret_array['where'] = str_replace("jt0.last_name like", " jt0.user_name LIKE '%{$_REQUEST['reports_to_name_advanced']}%' OR CONCAT(jt0.first_name, ' ', jt0.last_name) LIKE ", $ret_array['where']);
        }
        /* APX Custom Codes - END for Ontrack #1795 */

        // Custom codes to modify Popup of User relate fields in Accounts Module (SAM and MDM) to filter based on ACL Roles Module
        if ($_REQUEST['action'] == 'Popup') {
            // Popup returns an SQL array instead of string
            if ((isset($_REQUEST['filter_sam_results']) && $_REQUEST['filter_sam_results']) || (strpos($_REQUEST['request_data'], 'users_accounts_1_name') !== false)) {
                $ret_array['from'] .= "
                    ...
                    INNER JOIN acl_roles 
                        ON acl_roles_users.role_id = acl_roles.id
                            ...
                            AND acl_roles.name = 'Strategic Account Manager'
                ";
            }

            // Popup returns an SQL array instead of string
            if ((isset($_REQUEST['filter_mdm_results']) && $_REQUEST['filter_mdm_results']) || (strpos($_REQUEST['request_data'], 'users_accounts_2_name') !== false)) {
                $ret_array['from'] .= "
                    INNER JOIN acl_roles_users 
                        ON acl_roles_users.user_id = users.id 
                            AND acl_roles_users.deleted = 0
                    INNER JOIN acl_roles 
                        ON acl_roles_users.role_id = acl_roles.id
                            AND acl_roles.deleted = 0
                            AND acl_roles.name = 'Market Development Manager'
                ";
            }
        }
