    ...
    public function action_map_markers()
    {
        ...
        if (!empty($_REQUEST['record']) ||
            ...
            if(isset($_REQUEST['my_items']) && $_REQUEST['my_items']) {
                ...
                if(!$current_user->isAdmin() && $map_module_type != 'Accounts') {
                    $newQuery = "
                    ...
                    $query .= " AND (securitygroups_users.user_id = '".$current_user->id."' OR securitygroups.assigned_user_id = '".$current_user->id."') AND securitygroups_cstm.type_c = 'Account Access' AND securitygroups.deleted = 0 AND securitygroups_users.deleted = 0";
                    $query .= " GROUP BY ".strtolower($map_module_type).".id ";
                }
            }

            if ($map_module_type == 'Accounts') {
                $newQuery = "WHERE (" . strtolower($map_module_type).".account_type = 'Prospect' OR " .strtolower($map_module_type).".account_type = 'Customer') AND ";
                $query = str_replace('where', $newQuery, $query);
            }
...
//        echo "</pre>";
        ...
        // APX Custom Codes: OnTrack #1180 - Quick Radius map not showing -- START
        if($module_type == 'Accounts'){
            $marker['address'] = $this->get_account_address($display);
        }
        // APX Custom Codes: OnTrack #1180 - Quick Radius map not showing -- END
        ...
        // APX Custom Codes -- START
        if (isset($display['account_type'])) {
            $marker['account_type'] = $display['account_type'];
        }
        // APX Custom Codes -- END
    ...
    // APX Custom Codes: OnTrack #1180 - Quick Radius Map address not shwoing correctly for Accounts -- START
    private function get_account_address($display)
    {
        $result = '';

        if(!empty($display)){
            if(!empty($display['billing_address_street'])){
                $result .= "{$display['billing_address_street']}, ";
            }

            if(!empty($display['billing_address_city'])){
                $result .= "{$display['billing_address_city']}, ";
            }

            if(!empty($display['billing_address_state'])){
                $result .= "{$display['billing_address_state']}, ";
            }

            if(!empty($display['billing_address_postalcode'])){
                $result .= "{$display['billing_address_postalcode']}, ";
            }

            if(!empty($display['billing_address_country'])){
                $result .= "{$display['billing_address_country']} ";
            }
        }

        return $result;
    }
    // APX Custom Codes: OnTrack #1180 - Quick Radius Map address not shwoing correctly for Accounts -- END
