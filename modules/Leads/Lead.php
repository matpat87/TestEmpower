...
class Lead extends Person implements EmailInterface
{
    ...
    */
    ...
    // APX Custom Codes: Modified Email Notification Body -- START
    public function set_notification_body($xtpl, $lead)
	{
		global $app_list_strings, $locale;

		$primaryAddressArray = [$lead->primary_address_street, $lead->primary_address_city, $lead->primary_address_state, $lead->primary_address_country];
		$primaryAddress = implode(" ", $primaryAddressArray);
		...
		$xtpl->assign("LEAD_TITLE", $lead->title);
		$xtpl->assign("LEAD_ACCOUNT_NAME", $lead->account_name);
		$xtpl->assign("LEAD_COMPANY_TYPE", (isset($lead->company_type_c) ? $app_list_strings['manufacturing_type_list'][$lead->company_type_c] : ""));
		$xtpl->assign("LEAD_SPECIFIC_AREA_OF_INTEREST", (isset($lead->specific_area_of_interest_c) ? $app_list_strings['specific_area_of_interest_list'][$lead->specific_area_of_interest_c] : ""));
		$xtpl->assign("LEAD_PRIMARY_ADDRESS", $primaryAddress);
		$xtpl->assign("LEAD_EMAIL_ADDRESS", $lead->email1);
    ...
    public function save($check_notify = false)
    {
        ...
        // APX Custom Codes: OnTrack #1440 - Enable Email Notifications -- START
        $check_notify = true;
        $GLOBALS['check_notify'] = true;
        // APX Custom Codes: OnTrack #1440 - Enable Email Notifications -- END
