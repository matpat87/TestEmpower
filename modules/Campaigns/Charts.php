    ...
    public function campaign_response_by_activity_type($datay= array(), $targets=array(), $campaign_id= null, $cache_file_name='a_file', $refresh=false, $marketing_id='')
    {
        ...
        if ($campaign_id) {
            ...
            if (!file_exists($xmlFile) || $refresh == true) {
                ...
                // APX Custom Codes: Fix Campaign Log Chart Count -- START
                // Changed count(*) hits to custom counter
                // $query = "SELECT activity_type,target_type, count(*) hits ";
                $query = "SELECT activity_type,target_type, 
                    (SELECT COUNT(DISTINCT (cl.target_id)) FROM campaign_log AS cl WHERE cl.campaign_id = campaign_log.campaign_id AND cl.activity_type = campaign_log.activity_type AND cl.target_type = campaign_log.target_type AND cl.marketing_id = campaign_log.marketing_id AND cl.archived=0 AND cl.deleted=0) AS hits 
                ";
                $query .= " FROM campaign_log ";
                $query .= " WHERE campaign_id = '$campaign_id' AND archived=0 AND deleted=0";
                // APX Custom Codes: Fix Campaign Log Chart Count -- END
                ...
                // APX Custom Codes: Add Negative Campaign Logs to Chart Data -- START
                $this->retrieveNegativeCampaignLogData($leadSourceArr, $total, $total_targeted,'not_viewed', 'viewed', $campaign_id, $marketing_id);
                $this->retrieveNegativeCampaignLogData($leadSourceArr, $total, $total_targeted, 'not_link', 'link', $campaign_id, $marketing_id);
                // APX Custom Codes: Add Negative Campaign Logs to Chart Data -- END
...
//            if (empty($opp_data['opp_count'])) $opp_data['opp_count']=0;
    ...
    // APX Custom Codes: Add Negative Campaign Logs to Chart Data
    public function retrieveNegativeCampaignLogData(&$leadSourceArr, &$total, &$total_targeted,  $activityTypeName, $activitySubType, $campaignId, $marketingId = '')
    {
        global $log;
        ...
        if (! empty($marketingId)) {
            $marketingQuery = " AND campaign_log.marketing_id = '{$marketingId}'";
        }

        // For some reason activity_type = 'link' needs to have >=1 to have correct counting
        $count = ($activitySubType <> 'link')
            ? "COUNT(*) = 1"
            : "COUNT(*) >= 1";

        $query = "SELECT 
                    '{$activityTypeName}' AS activity_type,
                    target_type,
                    SUM(hits) AS hits
                FROM (
                    SELECT 
                        campaign_log.target_type,
                        COUNT(DISTINCT(campaign_log.target_id)) AS hits
                    FROM
                        campaign_log
                    WHERE campaign_log.campaign_id = '{$campaignId}'
                        AND campaign_log.deleted = 0
                        AND campaign_log.activity_type = 'targeted'
                        AND campaign_log.archived = 0
                        {$marketingQuery}
                        AND campaign_log.target_id NOT IN (
                            SELECT 
                                cl.target_id
                            FROM
                                campaign_log cl
                            WHERE cl.campaign_id = campaign_log.campaign_id
                                AND cl.deleted = 0
                                AND cl.activity_type = '{$activitySubType}'
                                AND cl.archived = 0
                                AND cl.marketing_id = campaign_log.marketing_id
                        )
