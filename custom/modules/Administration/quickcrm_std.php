...
$QuickCRMDetailsFields = array(
	...
	'Calls' => array(
			'name','direction','status','date_start','duration_hours','duration_minutes','assigned_user_name','description','parent_name','reminder_time',
		),
	'Meetings' => array(
			'name','status','date_start','duration_hours','duration_minutes','assigned_user_name','description','parent_name','reminder_time',
		),
	'Tasks' => array(
			'name','status','date_start','date_due','priority','assigned_user_name','description','contact_name','parent_name','assigned_user_name'
		),
	'Notes' => array(
	...
	'Cases' => array(
		'name','case_number','status','priority','description','account_name','assigned_user_name'
		),
	'Emails' => array(
	...
	'ProjectTask' => array(
			'name','status','priority','project_name','description','assigned_user_name'
	),
	'Employees' => array(
			'first_name',
			'last_name',
	...
	'CampaignLog' => array(
		'campaign_name1','activity_type','activity_date'
		),
); 
...
$QuickCRMDefSearch = array(
	...
	'Tasks' => array(
			'status','date_due','priority','assigned_user_name'
		),
	'Cases' => array(
...
$QuickCRMDefList = array(
	...
	'Cases' => array(
		'case_number','account_name','status','priority',
		),
	'ProjectTask' => array(
			'status','priority',"assigned_user_name"
	),
	...
	'QCRM_SavedSearch' => array(
			"name","description","fields",(($sugar_config['dbconfig']['db_type'] =='mssql')?"shared_rec":"shared")
		),
	'QCRM_Homepage' => array(
			"name","description",(($sugar_config['dbconfig']['db_type'] =='mssql')?"shared_rec":"shared"),"charts","dashlets","icons","hidden","creates"
		),
); 
...
$QuickCRMExtraFields = array(// field definitions required by the app
	...
	'Meetings' => array(
			'duration','duration_hours','duration_minutes','date_end','reminder_time','reminder_checked','repeat_type','repeat_interval','repeat_count','repeat_dow','repeat_until','recurring_source','repeat_parent_id',
		),
	'Tasks' => array(
			'date_start','date_due',
		),
...
if ($found_aos){
	...
	$QuickCRMDetailsFields['AOS_Quotes'] = array(
			'name','number','billing_account','billing_contact','stage',"expiration",'total_amt','total_amount',
			//'name','number','stage',"expiration",'billing_account','billing_contact','$ADDbilling','$ADDshipping',"line_items",'total_amt','discount_amount','subtotal_amount','tax_amount','total_amount',
		);
	...
	$QuickCRMExtraFields['AOS_Quotes'] = array(
			"number","line_items","total_amt","discount_amount","subtotal_amount","subtotal_tax_amount","tax_amount","tax_amount","total_amount"
		);
	...
	$QuickCRMDetailsFields['AOS_Invoices'] = array(
			'name','number','billing_account','billing_contact','invoice_date','status','total_amt','total_amount',
			//'name','number','invoice_date','status','billing_account','billing_contact','$ADDbilling','$ADDshipping',"line_items",'total_amt','discount_amount','subtotal_amount','tax_amount','total_amount',
		);
	...
	$QuickCRMTitleFields['AOS_Contracts'] = array(
			'name',
		);
		
	$QuickCRMDetailsFields['AOS_Contracts'] = array(
			'name','reference_code','contract_account','contact','start_date','end_date','status','total_amt','total_amount',
			//'name','number','invoice_date','status','billing_account','billing_contact','$ADDbilling','$ADDshipping',"line_items",'total_amt','discount_amount','subtotal_amount','tax_amount','total_amount',
		);
		
	$QuickCRMDefList['AOS_Contracts'] = array(
			'number','contract_account','total_amt'
		);
		
	$QuickCRMDefSearch['AOS_Contracts'] = array(
			'status',
		);
		
	$QuickCRMDefColors['AOS_Contracts'] = 'status';

	$QuickCRMExtraFields['AOS_Contracts'] = array(
			"number","line_items","total_amt","discount_amount","subtotal_amount","subtotal_tax_amount","tax_amount","tax_amount","total_amount"
		);
		
	$QuickCRMDefSubPanels['AOS_Contracts'] = array(
		);
...
*/		
	...
	if ($aos_version > '5.3') {
		...
		array_push($QuickCRMExtraFields['AOS_Contracts'],"shipping_amount","shipping_tax_amt","shipping_tax");
...
if (isset($sugar_config['suitecrm_version'])){
	$QuickCRMDefEdit['Cases'] = array(
		'name','case_number','state','status','priority','description','account_name','assigned_user_name','update_text','internal',
	);
	$QuickCRMDetailsFields['Cases'] = array(
		'name','case_number','state','status','priority','description','account_name','assigned_user_name','aop_case_updates_threaded'
	);
	$QuickCRMDefSearch['Cases'] = array(
			'state','status','priority','assigned_user_name'
	);
}
