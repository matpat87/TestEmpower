	...
	<tr id='pagination' class="pagination-unique" role='presentation'>
		<td colspan='{if $prerow}{$colCount+1}{else}{$colCount}{/if}'>
			<table border='0' cellpadding='0' cellspacing='0' width='100%' class='paginationTable'>
				<tr>
                    <td nowrap="nowrap" class='paginationActionButtons' style='border-top-left-radius: 4px;'> {* APX Custom Codes: Added Style *}
						{if $prerow}
      ...
      {/if}
                    ...
                    {* APX Custom Code if-else block for $customPagination when set to TRUE will display the custom pagination with go-to-page feature ELSE will display the default layout.
                      NOTE: anything in the ELSE block is the default(core) layout.
                  *}
                    {if $customPagination}
                    {include file='themes/SuiteP/include/ListView/CustomListViewPagination.tpl'}
                    {else}
