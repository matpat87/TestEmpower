    ...
    public function save_lines($post_data, $parent, $groups = array(), $key = '')
    {
        ...
        for ($i = 0; $i < $line_count; ++$i) {
            if (isset($post_data[$key . 'deleted'][$i]) && $post_data[$key . 'deleted'][$i] == 1) {
                ...
                // APX Custom Codes: OnTrack #704 - Invoice module overhaul - bypass saving for Invoice module only -- START
                if($parent->module_name == 'AOS_Invoices' && $product_quote->id == ''){
                    $product_quote->product_id = create_guid();
                    $product_quote->product_unit_price = 0;
                    $product_quote->id = $product_quote->product_id;
                    $product_quote->new_with_id = true;
                }
                // APX Custom Codes: OnTrack #704 - Invoice module overhaul - bypass saving for Invoice module only -- END
