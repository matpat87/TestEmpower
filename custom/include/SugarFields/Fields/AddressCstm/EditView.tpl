...
{{assign var="street2" value=$displayParams.key|cat:'_address_street2_c'}}
...
<fieldset id='{{$key}}_address_fieldset'>
    ...
    <table border="0" cellspacing="1" cellpadding="0" class="edit" width="100%">
        ...
        <tr>
            <td valign="top" id="{{$street2}}_label" width='25%' scope='row'>
                <label for='{{$street2}}'>{sugar_translate label='LBL_STREET' module='{{$module}}'}:</label>
                {if $fields.{{$street2}}.required || {{if $street2|lower|in_array:$displayParams.required}}true{{else}}false{{/if}}}
                <span class="required">{$APP.LBL_REQUIRED_SYMBOL}</span>
                {/if}
            </td>
            <td width="*">
                {{if $displayParams.maxlength}}
                <textarea id="{{$street2}}" name="{{$street2}}" maxlength="{{$displayParams.maxlength}}"
                          rows="{{$displayParams.rows|default:4}}"
                          cols="{{$displayParams.cols|default:60}}" {{if !empty($tabindex)}}
                          tabindex="{{$tabindex}}" {{/if}} {{if !empty($displayParams.accesskey)}}
                          accesskey='{{$displayParams.accesskey}}' {{/if}}
                          style="min-height: 110px;" >{$fields.{{$street2}}.value}</textarea>
                {{else}}
                <textarea id="{{$street2}}" name="{{$street2}}" rows="{{$displayParams.rows|default:4}}"
                          cols="{{$displayParams.cols|default:60}}" {{if !empty($tabindex)}}
                          tabindex="{{$tabindex}}" {{/if}} {{if !empty($displayParams.accesskey)}}
                          accesskey='{{$displayParams.accesskey}}' {{/if}}
                          style="min-height: 110px;" >{$fields.{{$street2}}.value}</textarea>
