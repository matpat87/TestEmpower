...
class Employee extends Person
{
    ...
    */
    ...
    public function create_new_list_query($order_by, $where, $filter=array(), $params=array(), $show_deleted = 0, $join_type='', $return_array = false, $parentbean=null, $singleSelect = false, $ifListForExport = false)
    {
        ...
        /* APX Custom Codes: Advanced Search query if reports to field has value -- START */
        if ($_REQUEST['module'] == 'Employees' && isset($_REQUEST['reports_to_name_advanced'])) { 
            $result['where'] = str_replace("jt0.last_name like", " jt0.user_name LIKE '%{$_REQUEST['reports_to_name_advanced']}%' OR CONCAT(jt0.first_name, ' ', jt0.last_name) LIKE ", $result['where']);
        }
        /* APX Custom Codes: Advanced Search query if reports to field has value -- START */

        //return parent method, specifying for array to be returned
        return $result; // APX Custom Codes: return $result instead of parent::create_new_list_query
    }
    ...
    public function verifyCurrentUserIsReportToUser()
    {
        ...
        $query = $db->query("
            ...
            FROM
                users
                    LEFT JOIN
                users_cstm ON users.id = users_cstm.id_c
                    LEFT JOIN
                users jt0 ON users.reports_to_id = jt0.id
                    AND jt0.deleted = 0
            WHERE 
                users.reports_to_id = '{$current_user->id}'
        ");
        ...
        if (in_array($this->id, $userIds) || in_array($this->reports_to_id, $userIds) || $current_user->id == $this->reports_to_id) {
            return true;
        }
