...
function DecodePanel($str){
	return trim(str_replace('XX',',',str_replace('_',' ',$str)));
}
...
function copyFromStudio($module,$profile_id,$view){
	global $qutils;
	$res = $qutils->getCRMFields($view,$module);
	$qutils->SetProfileView($module,$profile_id,$view,$res['fields']);
/*
	if ($view == 'fields' && $_POST['syncCheckbox'] =='0'){
		$res_detail = $qutils->getCRMFields('detail',$module);
		$qutils->SetProfileView($module,$profile_id,'detail',$res_detail['fields']);
	}
*/	
	$ret_array = array();
	if (count($res['warning']) > 0){
		$ret_array['warning']= 'Some fields could not be inserted';
		$ret_array['warning_details']= $res['warning'];
	}
	return $ret_array;
}
...
if (isset($_POST['act']) && $_POST['act'] ='fromStudio') {
	$warning = copyFromStudio($_POST['conf_module'],$profile_id,$_POST['conf_type']);
	if (isset($res['warning'])){
		$warning = $res['warning'];
		$warning_details = $res['warning_details'];
	}
}
else if ($_POST['conf_module']!='general') {
...
$res['warning']= $warning;
$res['warning_details']= $warning_details;
