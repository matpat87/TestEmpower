...
{* APX Custom Codes: OnTrack #644 - Force Password Change -- START *}
<table class="table" cellpadding="0" align="center" width="100%" cellspacing="0" border="0">
<tr>
<td>
<table class="table" cellpadding="0"  cellspacing="0" border="0" align="center">
<tr>
<td style="padding-bottom: 10px;" >
<img src="{$sugar_md}" alt="SuiteCRM" style="width: 240px; height: 120px;" /></td>
{* APX Custom Codes: OnTrack #644 - Force Password Change -- END *}
</tr>
<tr>
<td align="center">
	{* APX Custom Codes: OnTrack #644 - Force Password Change -- START *}
	<table id="tbl-change-pass" class="table" cellpadding="0" cellspacing="2" border="0" align="center" width="100%" class="edit view">
        <tbody>
            <tr>
            ...
            <tr>
                {assign var="lblNewPassword" value=$MOD.LBL_NEW_PASSWORD}
                {if empty($lblNewPassword)}
                    {assign var=lblNewPassword value=$LBL_NEW_PASSWORD}
                {/if}

                <td scope="row">{$lblNewPassword}:</td>
                <td><div class="col-md-4"><input class="form-control" type="password" size="26" tabindex="2" id="new_password" name="new_password" value="" onkeyup="password_confirmation();newrules('{$PWDSETTINGS.minpwdlength}','{$PWDSETTINGS.maxpwdlength}','{$REGEX}');" /></div></td>
            </tr>
            <tr>
                {assign var="lblNewPassword2" value=$MOD.LBL_NEW_PASSWORD2}
                {if empty($lblNewPassword2)}
                    {assign var=lblNewPassword2 value=$LBL_NEW_PASSWORD2}
                {/if}

                <td scope="row">{$lblNewPassword2}:</td>
                <td><div class="col-md-4"><input class="form-control" type="password" size="26" tabindex="2" id="confirm_pwd" name="confirm_pwd" value="" onkeyup="password_confirmation();" /> <div id="comfirm_pwd_match" class="error" style="display: none;">{$MOD.LBL_PASSWORD_MIS_MATCH}</div></div></td>
            </tr>
            <tr>
                <td>{$CAPTCHA}</td>
            </tr>
            <tr>
                <td colspan="2"><div class="col-md-1">{$SUBMIT_BUTTON}</div></td>
