                ...
                if ($field_value != '' || $operator == 'isnull') {
                    ...
                    foreach ($parms['db_field'] as $db_field) {
                        ...
                        if ($type == 'datetime' || $type == 'datetimecombo') {
                            try {
//                                $field_value = $timedate->to_db_date($field_value, false); // APX Custom Code: Bug fix for date search issue in Advanced Filter using between operator (ontrack 1997)
