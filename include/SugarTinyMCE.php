...
eoq;
    ...
    private function overloadButtonConfigs()
    {
        if (file_exists($this->customConfigFile)) {
            // APX Custom Code: Changed from require_once to include as it only applies to first WYSIWYG/HTML field -- START
            // require_once($this->customConfigFile);
            include($this->customConfigFile);
            // APX Custom Code: Changed from require_once to include as it only applies to first WYSIWYG/HTML field -- END
    ...
    private function overloadDefaultConfigs()
    {
        if (file_exists($this->customDefaultConfigFile)) {
            ...
            include($this->customDefaultConfigFile);
            // APX Custom Code: Changed from require_once to include as it only applies to first WYSIWYG/HTML field -- END
