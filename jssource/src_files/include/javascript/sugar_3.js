  ...
  try {
    ...
    if (!raiseFlag) {
      ...
      if (flash)
        ...
        inputHandle.style.borderWidth = "1px";
        inputHandle.style.borderColor = "#FF0000";
  ...
  for (var wp = 0; wp < inputsWithErrors.length; wp++) {
    if (typeof(inputsWithErrors[wp]) != 'undefined' && typeof inputsWithErrors[wp].parentNode != 'undefined' && inputsWithErrors[wp].parentNode != null) {
      if (inputsWithErrors[wp].parentNode.className.indexOf('x-form-field-wrap') != -1) {
        ...
        inputsWithErrors[wp].removeAttribute('style'); // APX Custom Codes
  ...
  return {
    ...
    handleForm: function () {
      ...
      if (sortableColumnsCount < 1 ) {
        /**
         * APX CUSTOM CODE: commented out hide ORDER BY FIELD in Advanced Search filter
         * to display field in all modules
...
SUGAR.searchForm = function () {
  ...
  return {
    ...
    clear_form: function (form, skipElementNames) {
      ...
      // APX Custom Codes: OnTrack #1330 - Column Chooser Keep Changes -- START
      // SUGAR.savedViews.clearColumns=true;
      // APX Custom Codes: OnTrack #1330 - Column Chooser Keep Changes -- END
    },
