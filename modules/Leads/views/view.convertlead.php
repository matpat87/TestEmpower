...
class ViewConvertLead extends SugarView
{
    ...
    public function display()
    {
        ...
        echo "</div>";
        echo "<script src='custom/modules/Leads/js/custom-edit.js'></script>"; // APX Custom Codes
    ...
    protected function handleSave()
    {
        ...
        global $beanList, $app_list_strings, $sugar_config; // APX Custom Codes: Added $app_list_strings, $sugar_config
        $this->loadDefs();
        $beans = array();
        $selectedBeans = array();
        $selects = array();
        ...
        foreach ($beans as $bean) {
            ...
            if (! $lead->division_c) {
                $createdUserBean = BeanFactory::getBean("Users", $lead->created_by);
                $lead->division_c = $createdUserBean->division_c;
            }
            
            // Set record division values to logged user's division if value is empty (Ex. division field is hidden on views)
            if (! $bean->division) {
                $bean->division = $lead->division_c;
            }

            if (! $bean->division_c) {
                $bean->division_c = $lead->division_c;
            }

            // Set division value based on bean (division, division_c) if exists, or set to logged user's division
            if ($bean->division) {
                $division = array_key_exists($bean->division, $app_list_strings['user_division_list']) ? $bean->division : $lead->division_c;
            } else if ($bean->division_c) {
                $division = array_key_exists($bean->division_c, $app_list_strings['user_division_list']) ? $bean->division_c : $lead->division_c;
            } else {
                $division = $lead->division_c;
            }
            ...
            if ($division) {
                ...
                if ($securitygroupDivisionBean) {                
                    SecurityGroupHelper::insertOrDeleteSecurityGroupRecord('insert', $securitygroupDivisionBean->id, $bean->id, $bean->module_name, $eventLogId);
                };
            }
        ...
        if (!empty($lead)) {	//Mark the original Lead converted
            ...
            if (!empty($lead->id) && !empty($lead->photo) &&
                ...
                $bCopied = false;
                // APX Custom Codes: Ontrack #1833 - Document storage configuration options -- START
                if (($lead->photo === $beans['Contacts']->photo) && is_readable($sugar_config['upload_dir'] . $lead->id . '_photo')) {
                    $bCopied = copy(
                        $sugar_config['upload_dir'] . $lead->id . '_photo',
                        $sugar_config['upload_dir'] . $beans['Contacts']->id . '_photo'
                    );
                }
                if ($bCopied) {
                    ...
                    LoggerManager::getLogger()->warning("Lead conversion: failed copying {$sugar_config['upload_dir']}" .
                        $lead->id . "_photo to {$sugar_config['upload_dir']}" . $beans['Contacts']->id . '_photo');
                }
                // APX Custom Codes: Ontrack #1833 - Document storage configuration options -- END
            }
        }
