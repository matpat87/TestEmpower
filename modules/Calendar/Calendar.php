...
//								 "ProjectTask" => array("showCompleted" => true,"start" =>  "date_start", "end" => "date_finish"),
    ...
    public $calendar_user; // APX Custom Codes
    ...
    public function __construct($view = "agendaWeek", $time_arr = array())
    {
        ...
        if (isset($_REQUEST['parentTab']) && $_REQUEST['parentTab'] == 'All') {
			$current_user->setPreference('calendar_user_filter_id', $current_user->id, 0, 'global', $current_user);
			$current_user->setPreference('shared_ids', [$current_user->id]);
			$this->calendar_user = $current_user;

			// Hide container and redirect user to Calendar with no parentTab parameter to fix data persistence issue
			echo '<style>
				#bootstrap-container, .expandedSidebar {
					display: none;
				}
			</style>';
			header("Refresh:0; url=index.php?module=Calendar&action=index");
		} else {
			$this->calendar_user = BeanFactory::getBean('Users', $current_user->getPreference('calendar_user_filter_id'));
        }
        ...
        $this->show_tasks = $this->calendar_user->getPreference('show_tasks');
        // APX Custom Codes -- END
        ...
        $this->show_calls = $this->calendar_user->getPreference('show_calls');
        // APX Custom Codes -- END
        ...
        $this->show_completed = $this->calendar_user->getPreference('show_completed');
        // APX Custom Codes -- END
        ...
        $this->day_start_time = $this->calendar_user->getPreference('day_start_time');
        // APX Custom Codes -- END
        ...
        $this->day_end_time = $this->calendar_user->getPreference('day_end_time');
        // APX Custom Codes -- END
        ...
        foreach ($this->acts_arr as $user_id => $acts) {
            if (isset($acts) && empty($acts)) {
                ...
                if ($shared_calendar_separate) {
                    ...
                    $this->items[ $this->calendar_user->id ][] = array();
                    // APX Custom Codes -- END
                }
                continue;
            }
            foreach ($acts as $act) {
                ...
                if ($shared_calendar_separate) {
                    ...
                    $this->items[ $this->calendar_user->id ][] = $item;
                    // APX Custom Codes -- END
                }
            }
            $i++;
    ...
    public function init_shared()
    {
        ...
        $user_ids = $this->calendar_user->getPreference('shared_ids');
        // APX Custom Codes -- END
        if (!empty($user_ids) && (is_countable($user_ids) ? count($user_ids) : 0) != 0 && !isset($_REQUEST['shared_ids'])) {
            ...
            if (isset($_REQUEST['shared_ids']) && (is_countable($_REQUEST['shared_ids']) ? count($_REQUEST['shared_ids']) : 0) > 0) {
                ...
                $this->calendar_user->setPreference('shared_ids', $_REQUEST['shared_ids']);
                // APX Custom Codes -- END
            } else {
                // APX Custom Codes -- START
                // $this->shared_ids = array($current_user->id);
                $this->shared_ids = array($this->calendar_user->id);
                // APX Custom Codes -- END
            }
        }
    }
