...
class Call extends SugarBean
{
    ...
    */
        ...
        $singleSelect = false, $ifListForExport = false)
    {
        ...
        if ((isset($_REQUEST['module']) && $_REQUEST['module'] == 'Calls')
        ...
        if(isset($_REQUEST['entryPoint']) && $_REQUEST['entryPoint'] == 'export'){
            $result = str_replace('FROM', ', contacts.id as contact_id_non_db FROM', $result);
        }
    ...
    private function get_parent_type_query(){
        ...
        foreach($app_list_strings['parent_type_display'] as $parent_type => $parent_type_val){
            ...
            if(!empty($parent_type)){
                ...
                $result['left_join'] .= "left join {$parent_table} {$parent_type_alias}{$i} on {$parent_type_alias}{$i}.id = calls.parent_id ";
