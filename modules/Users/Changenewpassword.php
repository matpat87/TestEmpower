    ...
    if (!empty($row) && $isValid === true) {
        ...
        if (!$expired) {
            ...
            if (isset($_REQUEST['login']) && $_REQUEST['login'] == '1') {
                if (strtolower($row['username']) == strtolower($_POST['user_name'])) {
...
// APX Custom Codes: OnTrack #1220 - Reset password -- START
...
$sugar_smarty->assign("LBL_NEW_PASSWORD", $mod_strings['LBL_NEW_PASSWORD']);
$sugar_smarty->assign("LBL_NEW_PASSWORD2", $mod_strings['LBL_NEW_PASSWORD2']);
// APX Custom Codes: OnTrack #1220 - Reset password -- END
