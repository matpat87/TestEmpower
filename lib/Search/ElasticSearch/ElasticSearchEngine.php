...
class ElasticSearchEngine extends SearchEngine
{
    ...
    private function createSearchParams(SearchQuery $query): array
    {
        ...
        /* APX Custom Codes: check for regex special characters in search string and append a \ to the character if any -- START 
            Ex. Search string: "20/20", query string should now be "20\/20"
        ...
        */
        $pattern = "/[!$^?\/|}{~:]/";
        $regex = preg_match($pattern, $searchStr);
        
        if ($regex) {
            $searchStr = preg_replace($pattern, '\\\${0}', $searchStr);
        }

        /* APX Custom Codes: check for regex special characters in search string and append a \ to the character if any -- END */
