...
class Document extends File
{
    ...
    public function save($check_notify = false)
    {
        ...
        if (empty($this->id) || $this->new_with_id) {
            ...
            if (!empty($_FILES['filename_file'])) {
                ...
                $beanId = (! isset($this->upload_source_id)) ? $this->id : $this->upload_source_id;
                rename("upload://{$beanId}", "upload://{$Revision->id}");
        ...
        $singleSelect = false, $ifListForExport = false)
    {
        ...
        // Temporarily sets $order_by to 'name' when $_REQUEST['Documents2_DOCUMENT_ORDER_BY'] so on trigger
         ...
         */
        if (isset($_REQUEST['Documents2_DOCUMENT_ORDER_BY']) && $_REQUEST['Documents2_DOCUMENT_ORDER_BY'] == 'parent_name_non_db') {
    ...
    private function customParentNameQuery()
    {
        ...
        $parentTypesSql = $db->query("SELECT DISTINCT(parent_type) FROM documents WHERE  documents.deleted = 0"); // Retrieve all parent_types existing in db table

        while ($row =  $db->fetchByAssoc($parentTypesSql)) {
            $tableName = !empty($row['parent_type']) ? strtolower($row['parent_type']) : '';

            if ($tableName != "") {
                $result['left_join'] .= "LEFT JOIN {$tableName} ON {$tableName}.id = documents.parent_id ";
              if (!in_array($tableName, $customParentNameModules)) {
                    $appendSelectString .= "WHEN documents.parent_type = '{$row['parent_type']}' THEN {$tableName}.name ";

                } else {
                    $appendSelectString .= "WHEN documents.parent_type = '{$row['parent_type']}' THEN CONCAT({$tableName}.first_name,' ',{$tableName}.last_name) ";
                }
            }
        }
