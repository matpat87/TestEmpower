...
class SugarWidgetSubPanelCustomerProductTopSelectButton extends SugarWidgetSubPanelTopButton
{
	...
	function display($widget_data, $additionalFormFields = NULL, $nonbutton = false)
	{
		...
		$casesBean = BeanFactory::getBean('Cases', $focus->id);
		if ($_REQUEST['module'] == "Cases" && ($_REQUEST['action'] == "DetailView" || $_REQUEST['action'] == "SubPanelViewer") && isset($casesBean) && isset($casesBean->id)) {
			$accountID = (isset($casesBean->account_id) && $casesBean->account_id != null) ? $casesBean->account_id : "";
			$customerProductID = (isset($casesBean->ci_customeritems_cases_1ci_customeritems_ida) && $casesBean->ci_customeritems_cases_1ci_customeritems_ida != null )
				? $casesBean->ci_customeritems_cases_1ci_customeritems_ida : "";

			$additionalParams = "&customer_product_id={$customerProductID}&account_id={$accountID}&return_module=Cases";
		}

		$initial_filter = "{$initial_filter}{$additionalParams}";
