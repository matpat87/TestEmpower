var commonMenuManagement = (function($) {
	...
	var addMenuItem = function(moduleId) {
		...
		var cell_roles = $('<i class="roles asol_icon icn-user" onclick="commonMenuManagement.menuItemRoles(this);" config="{}" title="'+language['roles']+'" /></i>')[0];
		...
		cell_Button.innerHTML += "<i class='asol_icon icn-trash' onClick='if(confirm(\""+language['alert']['delete']+"\")) { $(this).closest(\".asolCommonMenuRow\").remove(); }'></i>";
	...
	var formatMenuItems = function(moduleId) {
		...
		currentModuleMenus.find('tr.asolCommonMenuRow').each(function(index) {
			...
			var roles = window.JSON.parse(unescape($(this).find("i.roles").attr('config')));
			
			elementMenu.acls = roles.acls;
			elementMenu.roles = roles.roles;
			//***AlineaSol Premium***//
			if (typeof window.hasPremiumCommonFeatures == 'function') {
				elementMenu.properties = window.JSON.parse(unescape($(this).find("i.properties").attr('config')));
			}
			//***AlineaSol Premium***//
	...
	var menuItemRoles = function(th,aclModules) {
		...
		availableAclsForMenus.forEach(function(aclsForMenus) {
			var acls = aclModules && !Array.isArray(config.acls) ? config.acls[Object.keys(config.acls)[0]] : config.acls;
			availableAclsOptions += "<option value='"+aclsForMenus+"'  "+(acls.indexOf(aclsForMenus)>=0? 'selected':'')+">"+aclsForMenus+"</option>";
		});
		...
		if(aclModules) filtersHtml += 	'<td><select class="aclModule" size="1">'+controllerConfig.getModuleOptions(Object.keys(config.acls)[0])+'</select></td>';
	...
	var menuItemRolesSave = function(){
		...
		if( $('.aclModule').val() ){
			config.acls = {};
			config.acls[$('.aclModule').val()] = $("#menuItemRoles .menuAcls").val() != null ? $("#menuItemRoles .menuAcls").val() : [];
		}else config.acls = $("#menuItemRoles .menuAcls").val() != null ? $("#menuItemRoles .menuAcls").val() : [];
			
		config.roles = $("#menuItemRoles .menuRoles").val() != null ? $("#menuItemRoles .menuRoles").val() : [];
			
		if ((config.acls.length || Object.keys(config.acls).length) > 0 || config.roles.length > 0){ 	/*Icon Migration*/
			button.css('color', '#ead306');
		} else { 	
			button.css('color', '');
		}
