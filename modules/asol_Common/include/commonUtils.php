...
class asol_CommonUtils {
	...
	static public $common_version = "2.9.1";
...
?>';
	...
	function getJsLanguages() {
		...
		if( isset($sugar_config['asolInstallationName']) && isset($sugar_config['domain_name']) ){
		...
		if(!is_file(sugar_cached('jsLanguage/').$asolCacheDir."asol_Theme/".$GLOBALS['current_language'].".js")){
			...
			$cacheDir = create_cache_directory('jsLanguage/asol_Theme/');
		...
		return '<script src="cache/jsLanguage/'.$asolCacheDir.'asol_Theme/'.$GLOBALS['current_language'].'.js?v='.md5_file('cache/jsLanguage/'.$asolCacheDir.'asol_Theme/'.$GLOBALS['current_language'].'.js').'"></script>';
