    ...
    public function create_new_list_query(
        ...
        if (is_array($result)) {
            $result['where'] = str_replace(
                "project_task.assigned_user_id IS NULL OR project_task.assigned_user_id = ''", 
                "project_task.assigned_user_id IS NULL OR project_task.assigned_user_id = '' OR project_task.assigned_user_id = 'null' OR project_task.assigned_user_id = '0'",
                $result['where']
            );
            $result['order_by'] = str_replace('parent_name_non_db', 'project_task.id', $result['order_by']);
            $result['order_by'] = str_replace('parent_type_non_db', 'project_task_cstm.parent_type', $result['order_by']);

            $result['where'] = str_replace(
                "project_task.projecttask_id_c",
                "project_task_cstm.projecttask_id_c",
                $result['where']
            );
            
            $result['from'] = str_replace('project_task.parent_id', 'project_task_cstm.parent_id', $result['from']);
        ...
        if (isset($_REQUEST['parent_type_advanced']) 
            && isset($_REQUEST['parent_name_advanced']) 
            && $_REQUEST['parent_name_advanced'] != ''
            && $_REQUEST['parent_type_advanced'] == 'Project') {
        
            $union = " UNION {$result['select']} {$result['from_min']}";
            $union .= " LEFT JOIN users jt0 ON project_task.assigned_user_id=jt0.id ";
            $union .= " LEFT JOIN pwg_projectworkgroup ON pwg_projectworkgroup.id = project_task_cstm.parent_id ";
            $union .=" LEFT JOIN project jt1 ON jt1.id = pwg_projectworkgroup.parent_id ";
            $union_where = str_replace(
                "((jt1.name like '{$_REQUEST['parent_name_advanced']}%' ) AND ( project_task_cstm.parent_type like 'Project%'))",
                "(jt1.name LIKE '{$_REQUEST['parent_name_advanced']}%' AND ( project_task_cstm.parent_type like 'PWG_ProjectWorkgroup%'))",
                $result['where']
            );
            
            $result['where'] = $result['where'] . $union . $union_where;
        }
        return $result;
    }
