...
class ViewPopup extends SugarView
{
    ...
    public function display()
    {
        ...
        if (!empty($listViewDefs) && !empty($searchdefs)) {
            ...
            // APX Custom Codes: Disable security group filter when searching via popup -- START
            if (isset($_REQUEST['disable_security_groups_filter'])) {
                $popup->th->ss->assign("disable_security_groups_filter", $_REQUEST['disable_security_groups_filter']);
            }
            // APX Custom Codes: Disable security group filter when searching via popup -- END
