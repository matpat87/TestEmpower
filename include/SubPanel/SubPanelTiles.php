...
class SubPanelTiles
{
    ...
    public function display($showContainer = true, $forceTabless = false)
    {
        ...
        foreach ($tabs as $t => $tab) {
            ...
            // APX Custom Codes: OnTrack #1446 - override previously set values and set expanded_subpanels = true initially -- START
            $tabs_properties[$t]['expanded_subpanels'] = true;
            // APX Custom Codes: OnTrack #1446 - override previously set values and set expanded_subpanels = true initially -- END
            ...
            if ($tabs_properties[$t]['expanded_subpanels']) {
                ...
                // APX Custom Codes: OnTrack #1446 -- START
                /**
                 * OnTrack #1446 customization: counts the tr's if subpanel has rows, it sets $tabs_properties[$t]['expanded_subpanels'] = TRUE
                 * and False if Otherwise.
                 * Behaviour only expands subpanel if it has values on load Detail View for all modules
                 */
                $tbodyContents = array_map('trim', explode('<tbody>', $tabs_properties[$t]['subpanel_body']))[1];
                $tbodyContents = explode('</tbody>', $tbodyContents)[0];
                ...
                // APX Custom Codes: OnTrack #1446 -- END
