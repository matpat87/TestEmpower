...
if($_REQUEST['method'] == 'validate') {
    FFOutfittersLicense::validate();
} else if($_REQUEST['method'] == 'change') {
    FFOutfittersLicense::change();
} else if($_REQUEST['method'] == 'add') {
    FFOutfittersLicense::add();
} else if($_REQUEST['method'] == 'test') {
    //optional param: user_id - test if a specific user has access to the add-on
    //Sugar 6: /index.php?module=NS_Reassign&action=outfitterscontroller&method=test&to_pdf=1
    //Sugar 7: #bwc/index.php?module=NS_Reassign&action=outfitterscontroller&method=test&to_pdf=1
    
    $user_id = null;
    if(!empty($_REQUEST['user_id'])) {
        $user_id = $_REQUEST['user_id'];
    }
    $validate_license = FFOutfittersLicense::isValid($currentModule,$user_id,true);
