...
class SugarWebServiceImplv4 extends SugarWebServiceImplv3_1
{
    ...
    public function login($user_auth, $application = null, $name_value_list = array(), ...$args)
    {
        ...
        // APX Custom Codes: Implement Implicit Frontend Hotfix -- START
        // $isLoginSuccess = $authController->login($user_auth['user_name'], $user_auth['password'], array('passwordEncrypted' => true));
        // $usr_id=$user->retrieve_user_id($user_auth['user_name']);

        //implicit frontend
        if (substr($user_auth['user_name'], 0, 6) == "SESSID") {
            $sessid = substr($user_auth['user_name'], 6, strlen($user_auth['user_name']) - 6);
            $GLOBALS['log']->info('sessionid(' . $sessid . ') sent. check if authenticated');
            session_id($sessid);
            session_start();
            if (isset($_SESSION['authenticated_user_id'])) {
                $isLoginSuccess = true;
                $usr_id = $_SESSION['authenticated_user_id'];
                $GLOBALS['log']->info('found authenticated_user_id: ' . $usr_id);
            } elseif(isset($_SESSION['samlNameId'])) {
                $GLOBALS['log']->info('try by samlNameId: ' . $_SESSION['samlNameId']);
                $userrow = User::findUserPassword($_SESSION['samlNameId'], null,
                    "(portal_only IS NULL OR portal_only !='1') AND (is_group IS NULL OR is_group !='1') AND status !='Inactive'", false);
                if(!empty($userrow)) {
                    $isLoginSuccess = true;
                    $usr_id = $userrow['id'];
                    $GLOBALS['log']->info('found by samlNameId: ' . $usr_id);
                }
            } else {
                $GLOBALS['log']->info('no auth user found in session');
            }
        } else {
            $isLoginSuccess = $authController->login($user_auth['user_name'], $user_auth['password'], array('passwordEncrypted' => true));
            $usr_id=$user->retrieve_user_id($user_auth['user_name']);
        ...
        //implicitfrontend
        // APX Custom Codes: Implement Implicit Frontend Hotfix -- END
