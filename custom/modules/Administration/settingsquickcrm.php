...
// LOGO
$current_logomode = '';
if (isset($qutils->mobile['show_logo'])){
	$current_logomode = $qutils->mobile['show_logo'];
}
$logo_array = array (
	'' => $app_strings['LBL_NONE'],
	'left' => $mod_strings['LBL_LOGO_LEFT'],
	'center' => $mod_strings['LBL_LOGO_CENTER'],
	'right' => $mod_strings['LBL_LOGO_RIGHT'],
);
$logooptions ="";
foreach($logo_array as $value => $label){
	$logooptions .= "<option value='$value' ".($current_logomode == $value ? "selected='selected'" :"").'>' .$label . '</option>';
}

if (!empty($sugar_config['quickcrm_custom_logo'])){
	$currentLogoLink = $sugar_config['quickcrm_custom_logo'];
}
else {
...
//$currentlogo='<img style="margin:5px 0;max-height:80px;" src="'.$currentLogoLink.'">';
$currentlogo='';
 
$logo_background = '';
if (!empty($sugar_config['quickcrm_logo_background'])){
	$logo_background = 'background-color:'.$sugar_config['quickcrm_logo_background'].';';
}
...
$whatsapp_checked=($qutils->mobile['whatsapp']?' checked="checked"':'');
$phone_country_code = $qutils->mobile['phone_country_code'];
//$phone_country_code_title = $app_list_strings["moduleList"]["Calls"] . ':&nbsp;' . $mod_strings['LBL_DIALLING_CODE'];
$phone_country_code_title = 'WhatsApp:&nbsp;' . $mod_strings['LBL_DIALLING_CODE'];
		...
		<div id="confpage" style="padding-left: 15px">
			...
			<div id="confgeneral">
				...
				<table>
					...
					<tr>
						...
						<td width="40%" valign="top">
							<div style="float:left;"">
								<select id="logomode" name="logomode" >
									$logooptions
								</select>
							</div>
							<div id="currentlogo" style="float:left;display:inline-block;$logo_background">
								$currentlogo
							</div>
						</td>	
					</tr>
					...
					<tr>
						<td id="whatsapp_label" width="10%" valign="top" scope="col">
						<label for="whatsapp">WhatsApp</label>
						</td>
						<td width="40%" valign="top">
						<input id="whatsapp" type="checkbox" {$whatsapp_checked} title="" name="whatsapp" onchange="changeWhatsApp()">
						</td>	
					</tr>
					<tr class='whatsapp_only' style="display:none;">
						<td id="phone_country_code_label" width="10%" valign="top" scope="col">
						<label for="phone_country_code">{$phone_country_code_title}</label>
						</td>
						<td width="40%" valign="top">
						<input id="phone_country_code" type="text" title="" name="phone_country_code" placeholder="+1" value="$phone_country_code">
						</td>	
					</tr>
EOQ;
if ($suitecrm){
