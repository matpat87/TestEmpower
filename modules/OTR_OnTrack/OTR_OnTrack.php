...
class OTR_OnTrack extends Issue
{
    ...
    function set_notification_body($xtpl, $otr_ontrack)
    {
        ...
        $xtpl->assign("OTR_MODULE", $app_list_strings['module_dropdown_list'][$otr_ontrack->module_c]);
    ...
    public function create_new_list_query(
        ...
        if (is_array($result) && !empty($_REQUEST['lvso']) && isset($_REQUEST['OTR_OnTrack2_OTR_ONTRACK_ORDER_BY'])) {
            $sortByColumn = $_REQUEST['OTR_OnTrack2_OTR_ONTRACK_ORDER_BY'];
            switch ($sortByColumn) {
                case 'status':
