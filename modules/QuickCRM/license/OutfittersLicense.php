...
*/                
        ...
        public static function doValidate($thisModule,$key=null)
        {
            ...
            $response = QSugarOutfitters_API::call($thisModule,'key/validate',$data,'get');
        ...
        public static function change()
        {
            ...
            $response = QSugarOutfitters_API::call($currentModule,'key/change',$data);
        ...
        public static function add()
        {
            ...
            $response = QOutfittersLicense::doValidate($currentModule);
...
if (!class_exists('QSugarOutfitters_API'))
{
    class QSugarOutfitters_API
    {
        ...
        public static function call($module,$path,$custom_data=array(),$method='post')
        {
            ...
            if($info['http_code'] == 0)
            {
                $GLOBALS['log']->fatal('QSugarOutfitters_API::call(): Unable to validate license. Please configure the firewall to allow requests to '.$outfitters_config['api_url'].'/key/validate and make sure that SSL certs are up to date on the server.');
                return array(
                        'success' => false,
                        'result' => 'QSugarOutfitters_API::call(): Unable to validate the license key. Please configure the firewall to allow requests to '.$outfitters_config['api_url'].'/key/validate and make sure that SSL certs are up to date on the server.'
                    );
            }
            else if($info['http_code'] != 200)
            {
                $GLOBALS['log']->fatal('QSugarOutfitters_API::call(): HTTP Request failed: '.print_r($info,true));
                return array(
                    'success' => false,
                    'result' => $result
                );
