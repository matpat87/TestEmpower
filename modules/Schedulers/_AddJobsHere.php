...
function pruneDatabase()
{
    ...
    if (!empty($tables)) {
        foreach ($tables as $kTable => $table) {
            ...
            // APX Custom Codes: OnTrack #1825 - Exclude modules from Prune DB -- START
            // Accounts, Orders, Invoices, Customer Products, Product Masters, Vendors, Raw Materials, Lots, and Shipping Details
            $excludedTables = ['accounts', 'odr_salesorders', 'aos_invoices', 'ci_customeritems', 'aos_products', 'vend_vendors', 'rmm_rawmaterialmaster', 'apx_lots', 'apx_shippingdetails'];
