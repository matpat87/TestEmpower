...
class SecurityGroup extends SecurityGroup_sugar
{
    ...
    public static function getGroupWhere($table_name, $module, $user_id)
    {
        ...
        if ($module == 'SecurityGroups') {
            if ($_REQUEST['disable_security_groups_filter']) {
                return true;
            }
            ...
            // APX Custom Codes to bypass JOIN with Security Groups if Module is JJWG Maps and Accounts OR if $_REQUEST['disable_security_groups_filter'] is set -- START
            if ($_REQUEST['module'] == 'jjwg_Maps' && $module == 'Accounts' || $_REQUEST['disable_security_groups_filter']) {
                return true;
            }
            // APX Custom Codes to bypass JOIN with Security Groups if Module is JJWG Maps and Accounts OR if $_REQUEST['disable_security_groups_filter'] is set -- END

            // APX Custom Codes: removed secr.record_id = " . $table_name . '.id from WHERE to improve query performance -- START
            // Note: Only remove condition if not on DetailView or SubpanelViewer as removing permanently displays all records on List View
            $filterByRecordId = (isset($_REQUEST['action']) && ! in_array($_REQUEST['action'], ['DetailView', 'SubPanelViewer'])) ? " AND secr.record_id = {$table_name}.id " : '';
            ...
            return " EXISTS (SELECT  1
                  FROM    securitygroups secg
                       ...
                       WHERE secg.deleted = 0 {$filterByRecordId})";
            // APX Custom Codes: removed secr.record_id = " . $table_name . '.id from WHERE to improve query performance -- END
        }
    }

    // APX Custom Codes override getGroupUsersWhere to filter results based on SecurityGroups -- START

    // Gets the join statement used for returning all users that a given user is in the same group with.
    // This function is the original getGroupUsersWhere but renamed since it's only being used in custom_get_user_array
    public static function coreGetGroupUsersWhere($user_id)
    {
        $db = DBManagerFactory::getInstance();
        $quotedUserId = $db->quote($user_id);
    ...
    // APX Custom Codes -- Gets the join statement used for return users that reports to a certain user by way of security groups with type Sales Group
    /**
     * Gets the join statement used for returning all users that a given user is in the same group with.
     *
     * @param string $user_id
    ...
    public static function getGroupUsersWhere($user_id)
    {
        ...
        return " users.id in (
            ...
            left join securitygroups
				on securitygroups.id = sec.securitygroup_id
            ...
            where sec.deleted = 0
                ...
                and securitygroups.assigned_user_id = '$quotedUserId'
        ) OR users.id = '$quotedUserId' OR users.status <> 'Active'";
    }
    
    // APX Custom Codes override getGroupUsersWhere to filter results based on SecurityGroups -- END
    ...
    public static function inherit($focus, $isUpdate)
    {
        ...
        // APX Custom Codes to exclude log modules from inheritance to prevent DB bloatup -- START
        // Ex. 'AURL_ActivityUserReassignmentsLog', 'SGRL_SecurityGroupRecordsLog'
        if (strpos(strtolower($focus->module_dir), 'log') !== false) {
            return;
        }
        // APX Custom Codes to exclude log modules from inheritance to prevent DB bloatup -- END
...
order by securitygroups_users.primary_group desc ";
    ...
    // APX Custom Codes to improve system performance during delete
    ...
    /**
     * Ontrack 1811: extended SugarBean save() to add `false` parameter to avoid triggering email notifications
    */
